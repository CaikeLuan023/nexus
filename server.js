...
const ALLOWED_MIMES = new Set([
    ...
    'application/pdf',
    'video/mp4',
    'audio/mpeg', 'audio/ogg', 'audio/opus', 'audio/wav', 'audio/mp3',
...
const ALLOWED_EXTS = /\.(jpg|jpeg|png|gif|webp|pdf|bmp|mp4|mp3|ogg|oga|opus|wav|doc|docx|xls|xlsx|ppt|pptx|txt|csv|zip|rar)$/i;
...
function getOnlineList() {
    return [...onlineUsers.values()].map(u => ({ id: u.id, nome: u.nome, perfil: u.perfil, page: u.page, foto_url: u.foto_url || null }));
}
...
async function wahaFetch(endpoint, options = {}) {
    ...
    catch { return { status: res.status, data: { message: await res.text() } }; }
}
...
async function processarAutoResposta(msg) {
    ...
    for (const r of respostas) {
        const palavras = r.palavra_chave.toLowerCase().split(',').map(p => p.trim());
        if (palavras.some(p => texto.includes(p))) {
            const chatId = msg.from || msg.chatId;
            try { await enviarMensagemWhatsApp(chatId, r.resposta); } catch (e) { console.error('Auto-resp erro:', e.message); }
            break;
        }
    }
}
...
function atualizarSessao(db, sessaoId, nodeAtual, variaveis) {
    db.queryRun("UPDATE whatsapp_flow_sessoes SET node_atual = ?, variaveis = ?, atualizado_em = datetime('now','localtime') WHERE id = ?",
        [nodeAtual, JSON.stringify(variaveis), sessaoId]);
}

function finalizarSessao(db, sessaoId) {
    db.queryRun("UPDATE whatsapp_flow_sessoes SET status = 'finalizado', atualizado_em = datetime('now','localtime') WHERE id = ?", [sessaoId]);
}
...
async function processarFluxo(msg) {
    ...
    let sessao = db.queryGet("SELECT * FROM whatsapp_flow_sessoes WHERE chat_id = ? AND status = 'ativo' ORDER BY id DESC LIMIT 1", [chatId]);
    ...
    if (!sessao) {
        ...
        let flowData;
        try { flowData = JSON.parse(flow.dados_flow); } catch { return false; }
        ...
        const r = db.queryRun('INSERT INTO whatsapp_flow_sessoes (flow_id, chat_id, chat_nome, node_atual, variaveis) VALUES (?, ?, ?, ?, ?)',
    ...
    let variaveis;
    try { variaveis = JSON.parse(sessao.variaveis || '{}'); } catch { variaveis = {}; }
    ...
    while (shouldContinue && !needsInput && steps < 50) {
        ...
        if (!node) { finalizarSessao(db, sessao.id); return true; }
        ...
        switch (node.name) {
            ...
            case 'menu': {
                if (!aguardandoInput) {
                    ...
                    if (escolha >= 1 && escolha <= opcoes.length) {
                        ...
                        outputIdx = opcoes.findIndex(op => textoUsuario.toLowerCase().includes(op.texto.toLowerCase()));
                    }
                    if (outputIdx >= 0) {
                        ...
                        if (tentativas >= maxTentativas) {
                            ...
                            await enviarMensagemWhatsApp(chatId, 'Numero maximo de tentativas atingido. Atendimento encerrado.');
                            return true;
                        }
            ...
            case 'condicao': {
                ...
                let resultado = false;
                switch (operador) {
                    case '==': resultado = varValor === valor; break;
                    case '!=': resultado = varValor !== valor; break;
                    case 'contem': resultado = varValor.toLowerCase().includes(valor.toLowerCase()); break;
                    case 'nao_contem': resultado = !varValor.toLowerCase().includes(valor.toLowerCase()); break;
                    case 'existe': resultado = !!varValor; break;
                    case 'vazio': resultado = !varValor; break;
                }
            ...
            case 'integracao': {
                try {
                    ...
                    if (node.data.variavel_resultado) {
                        variaveis[node.data.variavel_resultado] = typeof respData === 'object' ? JSON.stringify(respData) : String(respData);
                    }
            ...
            case 'transferir': {
                const mensagem = substituirVariaveis(node.data.mensagem || 'Voce sera transferido para um atendente.', variaveis);
                await enviarMensagemWhatsApp(chatId, mensagem);
                db.queryRun("UPDATE whatsapp_flow_sessoes SET status = 'transferido', atualizado_em = datetime('now','localtime') WHERE id = ?", [sessao.id]);
...
setInterval(() => {
    try {
        ...
        db.queryRun("UPDATE whatsapp_flow_sessoes SET status = 'finalizado' WHERE status = 'ativo' AND atualizado_em < ?", [limite]);
    } catch {}
...
async function dispararNotificacao(tipo, dados) {
    ...
    try { await enviarMensagemWhatsApp(config.chat_id, texto); } catch (e) { console.error(`Notif ${tipo}:`, e.message); }
}
...
function requireModuleAccess(moduleName) {
    return (req, res, next) => {
        ...
        const perm = db.queryGet('SELECT ativo FROM permissoes_modulos WHERE perfil = ? AND modulo = ?',
            [req.session.usuario.perfil, moduleName]);
...
app.post('/api/login', loginLimiter, (req, res) => {
    ...
    if (typeof usuario !== 'string' || typeof senha !== 'string') return res.status(400).json({ erro: 'Dados invalidos' });
...
function salvarMensagemLocal(msgData) {
    try {
        const db = getDB();
        db.queryRun(`INSERT OR IGNORE INTO whatsapp_mensagens
            (message_id, chat_id, chat_name, from_me, body, type, sender_name, media_url, filename, timestamp, quoted_msg_id, quoted_msg_body)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [msgData.messageId, msgData.chatId, msgData.chatName || '', msgData.fromMe ? 1 : 0,
             msgData.body || '', msgData.type || 'chat', msgData.senderName || '',
             msgData.mediaUrl || null, msgData.filename || null,
             msgData.timestamp || Math.floor(Date.now() / 1000),
             msgData.quotedMsgId || null, msgData.quotedMsgBody || null]);
    } catch (e) { console.error('[db] erro ao salvar msg:', e.message); }
}
...
app.post('/api/whatsapp/webhook', async (req, res) => {
    ...
    if (event === 'message' && data.payload) {
        ...
        const normalized = {
            event: 'message',
            payload: {
                from: chatId, to: p.to || '', fromMe: p.fromMe || false,
                body: msgBody, senderName, type: msgType,
                timestamp: p.timestamp || Math.floor(Date.now() / 1000),
                id: msgId, chatName: senderName,
                isGroup: chatId.includes('@g.us'),
                hasMedia, _mediaUrl: mediaUrl, filename: p.media?.filename || null
            }
        };
        ...
        salvarMensagemLocal({
            messageId: msgId, chatId, chatName: senderName,
            fromMe: p.fromMe || false, body: msgBody, type: msgType,
            senderName, mediaUrl, filename: p.media?.filename || null,
            timestamp: p.timestamp || Math.floor(Date.now() / 1000),
            quotedMsgId: p.quotedMsg?.id ? (typeof p.quotedMsg.id === 'object' ? p.quotedMsg.id._serialized : p.quotedMsg.id) : null,
        ...
        try {
            ...
            db.queryRun('INSERT INTO whatsapp_metricas (tipo, chat_id, chat_nome) VALUES (?, ?, ?)',
                [p.fromMe ? 'enviada' : 'recebida', chatId, senderName]);
        } catch {}
        if (!p.fromMe) {
            ...
            if (!chatId.includes('@g.us')) {
                try {
                    const db2 = getDB();
                    const atendExistente = db2.queryGet("SELECT id, agente_id, status FROM whatsapp_atendimentos WHERE chat_id = ? AND status IN ('fila','em_atendimento')", [chatId]);
                    if (!atendExistente) {
                        const result = db2.queryRun("INSERT INTO whatsapp_atendimentos (chat_id, chat_nome, status) VALUES (?, ?, 'fila')",
                            [chatId, senderName || chatId.split('@')[0]]);
                        db2.queryRun("INSERT INTO whatsapp_atendimentos_log (atendimento_id, acao, detalhes) VALUES (?, 'criado', 'Mensagem recebida')",
                            [result.lastInsertRowid]);
                        broadcastSSE({ event: 'atendimento.novo', payload: { id: result.lastInsertRowid, chat_id: chatId, chat_nome: senderName || chatId.split('@')[0], status: 'fila' } });
                    }
                    // Anexar info de atendimento ao payload para filtragem client-side
                    const atendInfo = atendExistente || db2.queryGet("SELECT id, agente_id, status FROM whatsapp_atendimentos WHERE chat_id = ? AND status IN ('fila','em_atendimento')", [chatId]);
                    if (atendInfo) {
                        normalized.payload._atendimento = { id: atendInfo.id, agente_id: atendInfo.agente_id, status: atendInfo.status };
                    }
                } catch (e) { console.error('Erro fila atendimento:', e.message); }
            }
            ...
            (async () => {
                ...
                } catch (e) { console.error('Erro ao processar fluxo/auto-resposta:', e); }
            })();
        }
    }
...
app.post('/api/formulario/:token', formPublicLimiter, express.json(), (req, res) => {
    ...
    db.queryRun("UPDATE formularios_cadastro SET dados = ?, status = 'preenchido', preenchido_em = datetime('now','localtime') WHERE id = ?",
...
app.get('/proposta-pdf/:token', (req, res) => {
    ...
    try {
        db.queryRun('INSERT INTO vendas_propostas_views (proposta_id, ip, user_agent) VALUES (?, ?, ?)',
            [proposta.id, req.ip || req.connection?.remoteAddress || '', (req.headers['user-agent'] || '').substring(0, 255)]);
        db.queryRun('UPDATE vendas_propostas SET visualizacoes = COALESCE(visualizacoes, 0) + 1 WHERE id = ?', [proposta.id]);
        broadcastSSE({ event: 'proposta.visualizada', payload: { proposta_id: proposta.id, provedor_nome: proposta.provedor_nome } });
    } catch {}
...
app.get('/contrato-aceite/:token', (req, res) => {
    ...
    <div class="contrato-card">
        <div class="contrato-body">
            ...
            ${contrato.valor_mensal || contrato.valor_total ? `<div class="valor-destaque">
...
app.post('/api/contrato-aceite/:token', (req, res) => {
    ...
    db.queryRun('UPDATE vendas_contratos SET status = ?, assinado_em = datetime(\'now\',\'localtime\'), assinatura_ip = ?, assinatura_nome = ? WHERE id = ?',
        ['assinado', ip, nome.trim(), contrato.id]);
    broadcastSSE({ event: 'contrato.assinado', payload: { id: contrato.id, provedor_nome: contrato.provedor_nome, assinatura_nome: nome.trim() } });
    res.json({ ok: true });
});
...
app.get('/chamados', requireModuleAccess('chamados'), (req, res) => res.sendFile(path.join(__dirname, 'views', 'chamados.html')));
app.get('/treinamentos', requireModuleAccess('treinamentos'), (req, res) => res.sendFile(path.join(__dirname, 'views', 'treinamentos.html')));
app.get('/projetos', requireModuleAccess('projetos'), (req, res) => res.sendFile(path.join(__dirname, 'views', 'projetos.html')));
app.get('/historico', requireModuleAccess('historico'), (req, res) => res.sendFile(path.join(__dirname, 'views', 'historico.html')));
app.get('/provedores', requireModuleAccess('provedores'), (req, res) => res.sendFile(path.join(__dirname, 'views', 'provedores.html')));
app.get('/whatsapp', requireModuleAccess('whatsapp'), (req, res) => res.sendFile(path.join(__dirname, 'views', 'whatsapp.html')));
app.get('/atendimento', requireModuleAccess('whatsapp'), (req, res) => res.sendFile(path.join(__dirname, 'views', 'atendimento.html')));
app.get('/flow', requireModuleAccess('whatsapp'), (req, res) => res.sendFile(path.join(__dirname, 'views', 'flow.html')));
app.get('/vendas', requireModuleAccess('vendas'), (req, res) => res.sendFile(path.join(__dirname, 'views', 'vendas.html')));
app.get('/dashboard-vendedor', requireModuleAccess('dashboard_vendedor'), (req, res) => res.sendFile(path.join(__dirname, 'views', 'dashboard-vendedor.html')));
app.get('/usuarios', requireAdmin, (req, res) => res.sendFile(path.join(__dirname, 'views', 'usuarios.html')));
app.get('/configuracoes', requireAdmin, (req, res) => res.sendFile(path.join(__dirname, 'views', 'configuracoes.html')));
...
app.get('/fila-atendimento', requireModuleAccess('chamados'), (req, res) => res.sendFile(path.join(__dirname, 'views', 'fila-atendimento.html')));
app.get('/nps', requireAuth, (req, res) => res.sendFile(path.join(__dirname, 'views', 'nps.html')));
app.get('/ponto', requireAuth, requireModuleAccess('ponto'), (req, res) => res.sendFile(path.join(__dirname, 'views', 'ponto.html')));
...
app.post('/api/heartbeat', (req, res) => {
    ...
    onlineUsers.set(u.id, { id: u.id, nome: u.nome, perfil: u.perfil, foto_url: fotoUrl, lastSeen: Date.now(), page: req.body.page || '' });
...
app.get('/api/chat/nao-lidas', (req, res) => {
    ...
    const porUsuario = {};
    let total = 0;
    rows.forEach(r => { porUsuario[r.remetente_id] = r.total; total += r.total; });
...
app.get('/api/chat/:userId', (req, res) => {
    ...
    db.queryRun('UPDATE chat_mensagens SET lido = 1 WHERE remetente_id = ? AND destinatario_id = ? AND lido = 0', [outroId, meuId]);
...
app.post('/api/chat/enviar', (req, res) => {
    ...
    const result = db.queryRun(
        'INSERT INTO chat_mensagens (remetente_id, destinatario_id, texto) VALUES (?, ?, ?)',
        [req.session.usuario.id, destinatario_id, texto.trim()]
    );
    const msg = {
...
app.get('/api/me', (req, res) => {
    ...
    const permissoes = {};
    perms.forEach(p => { permissoes[p.modulo] = !!p.ativo; });
    // Admin sempre tem acesso a tudo + usuarios
    if (req.session.usuario.perfil === 'admin') {
        ['dashboard','provedores','vendas','dashboard_vendedor','chamados','treinamentos','projetos','historico','whatsapp','relatorios','conhecimento','agenda','financeiro','usuarios','configuracoes'].forEach(m => { permissoes[m] = true; });
    }
...
app.post('/api/regras-automaticas', requireAdmin, (req, res) => {
    ...
    const result = db.queryRun(
        ...
        [nome, tipo_gatilho, condicao_valor ? JSON.stringify(condicao_valor) : null, acao, acao_config ? JSON.stringify(acao_config) : null, req.session.usuario.nome]
    );
...
app.put('/api/regras-automaticas/:id', requireAdmin, (req, res) => {
    ...
    db.queryRun(
        ...
        [nome, tipo_gatilho, condicao_valor ? JSON.stringify(condicao_valor) : null, acao, acao_config ? JSON.stringify(acao_config) : null, ativo ? 1 : 0, Number(req.params.id)]
    );
        ...
        for (const regra of regras) {
            try {
                ...
                if (regra.tipo_gatilho === 'chamado_pendente_dias') {
                    ...
                    const chamados = db.queryAll(`SELECT * FROM chamados WHERE status = 'pendente' AND data_abertura <= datetime('now','localtime','-' || ? || ' days')`, [dias]);
                    for (const c of chamados) {
                        if (regra.acao === 'notificar') {
                            criarNotificacaoParaPerfil('admin', 'sistema', `Chamado pendente ha ${dias} dias`, `#${c.id} - ${c.titulo}`, '/chamados');
                        } else if (regra.acao === 'alterar_status') {
                            db.queryRun('UPDATE chamados SET status = ? WHERE id = ?', [acaoConfig.novo_status || 'em_andamento', c.id]);
                        }
                    }
                } else if (regra.tipo_gatilho === 'projeto_atrasado') {
                    const projetos = db.queryAll(`SELECT * FROM projetos WHERE status = 'em_andamento' AND data_previsao IS NOT NULL AND data_previsao < date('now','localtime')`);
                    for (const p of projetos) {
                        if (regra.acao === 'notificar') {
                            criarNotificacaoParaPerfil('admin', 'sistema', 'Projeto atrasado', `${p.titulo} - Previsao: ${p.data_previsao}`, '/projetos');
                        }
                    }
                }

                db.queryRun("UPDATE regras_automaticas SET ultima_execucao = datetime('now','localtime') WHERE id = ?", [regra.id]);
            } catch (e) { console.error(`Regra ${regra.id} erro:`, e.message); }
        }
    } catch (e) { console.error('Regras automaticas erro:', e.message); }
}
...
app.post('/api/tarefas-recorrentes', requireAdmin, (req, res) => {
    ...
    if (!titulo || !modulo || !frequencia) return res.status(400).json({ erro: 'Titulo, modulo e frequencia obrigatorios' });
    ...
    const result = db.queryRun(
        ...
        [titulo, descricao || null, modulo, frequencia, dia_semana || null, dia_mes || null, hora || '09:00', config ? JSON.stringify(config) : null, proxima, req.session.usuario.nome]
    );
...
app.put('/api/tarefas-recorrentes/:id', requireAdmin, (req, res) => {
    ...
    db.queryRun(
        ...
        [titulo, descricao || null, modulo, frequencia, dia_semana || null, dia_mes || null, hora || '09:00', config ? JSON.stringify(config) : null, ativo ? 1 : 0, proxima, Number(req.params.id)]
    );
...
function processarTarefasRecorrentes() {
    try {
        ...
        const tarefas = db.queryAll('SELECT * FROM tarefas_recorrentes WHERE ativo = 1 AND proxima_execucao <= ?', [now]);

        for (const tarefa of tarefas) {
            try {
                criarNotificacaoParaPerfil('admin', 'sistema', `Tarefa recorrente: ${tarefa.titulo}`, tarefa.descricao || 'Executar tarefa programada', '/configuracoes');

                const proxima = calcularProximaExecucao(tarefa.frequencia, tarefa.dia_semana, tarefa.dia_mes, tarefa.hora);
                db.queryRun("UPDATE tarefas_recorrentes SET ultima_execucao = datetime('now','localtime'), proxima_execucao = ? WHERE id = ?", [proxima, tarefa.id]);
            } catch (e) { console.error(`Tarefa recorrente ${tarefa.id} erro:`, e.message); }
        }
    } catch (e) { console.error('Tarefas recorrentes erro:', e.message); }
}
    ...
    res.end = function(...args) {
        ...
        try {
            ...
            db.queryRun(
                ...
                [req.method, req.originalUrl, res.statusCode, tempo, req.ip || '', (req.headers['user-agent'] || '').substring(0, 200), req.apiToken?.nome || null, res.statusCode >= 400 ? (res._logErro || null) : null]
            );
        } catch (e) { console.error('Log API request erro:', e.message); }
...
async function dispararWebhooks(evento, dados) {
        ...
        for (const wh of webhooks) {
            ...
            try {
                ...
                const resp = await fetch(wh.url, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json', ...(wh.headers_json ? JSON.parse(wh.headers_json) : {}) },
                    body: payload,
    ...
    } catch (e) { console.error('dispararWebhooks erro:', e.message); }
}
...
app.get('/api/v1/chamados', requireApiToken, requireApiPermission('leitura'), (req, res) => {
    const db = getDB();
    res.json(db.queryAll('SELECT c.*, p.nome as provedor_nome FROM chamados c JOIN provedores p ON c.provedor_id = p.id ORDER BY c.data_abertura DESC LIMIT 100'));
});
...
app.get('/api/v1/projetos', requireApiToken, requireApiPermission('leitura'), (req, res) => {
    const db = getDB();
    res.json(db.queryAll('SELECT pr.*, p.nome as provedor_nome FROM projetos pr LEFT JOIN provedores p ON pr.provedor_id = p.id ORDER BY pr.data_inicio DESC LIMIT 100'));
});

// Gerenciamento de tokens
app.get('/api/config/api-tokens', requireAdmin, (req, res) => {
    const db = getDB();
    const tokens = db.queryAll('SELECT id, nome, token, ativo, permissoes, ultimo_uso, criado_por, criado_em FROM api_tokens ORDER BY criado_em DESC');
    // Mask tokens: show only first 6 and last 4 chars
    tokens.forEach(t => { t.token = t.token.substring(0, 6) + '****' + t.token.slice(-4); });
    res.json(tokens);
});
...
app.post('/api/config/api-tokens', requireAdmin, (req, res) => {
    ...
    const result = db.queryRun('INSERT INTO api_tokens (nome, token, permissoes, criado_por) VALUES (?, ?, ?, ?)',
        [nome, token, permissoes || 'leitura', req.session.usuario.nome]);
...
app.post('/api/config/webhooks', requireAdmin, (req, res) => {
    ...
    const result = db.queryRun('INSERT INTO webhooks_saida (nome, url, eventos, secret, criado_por) VALUES (?, ?, ?, ?, ?)',
...
function registrarAtividade(req, acao, modulo, entidade_id, detalhes) {
    try {
        ...
        db.queryRun(
            ...
            [usuario_id, usuario_nome, acao, modulo, entidade_id, typeof detalhes === 'object' ? JSON.stringify(detalhes) : (detalhes || ''), ip]
        );
...
app.get('/api/atividades', requireAdmin, (req, res) => {
    ...
    if (usuario_id) { sql += ' AND usuario_id = ?'; params.push(Number(usuario_id)); }
...
app.get('/api/logs/resumo', requireAdmin, (req, res) => {
    ...
    const requestsHoje = db.queryGet("SELECT COUNT(*) as total FROM api_request_log WHERE criado_em >= ?", [hoje]) || { total: 0 };
    const errosHoje = db.queryGet("SELECT COUNT(*) as total FROM api_request_log WHERE criado_em >= ? AND status_code >= 400", [hoje]) || { total: 0 };
    const syncsHoje = db.queryGet("SELECT COUNT(*) as total FROM erp_sync_log WHERE criado_em >= ?", [hoje]) || { total: 0 };
    const webhooksHoje = db.queryGet("SELECT COUNT(*) as total FROM webhook_dispatch_log WHERE criado_em >= ?", [hoje]) || { total: 0 };
    const webhooksErroHoje = db.queryGet("SELECT COUNT(*) as total FROM webhook_dispatch_log WHERE criado_em >= ? AND sucesso = 0", [hoje]) || { total: 0 };
    const tempoMedio = db.queryGet("SELECT AVG(tempo_resposta_ms) as avg FROM api_request_log WHERE criado_em >= ?", [hoje]) || { avg: 0 };
    const erpCommHoje = db.queryGet("SELECT COUNT(*) as total FROM erp_communication_log WHERE criado_em >= ?", [hoje]) || { total: 0 };
    const erpCommErroHoje = db.queryGet("SELECT COUNT(*) as total FROM erp_communication_log WHERE criado_em >= ? AND sucesso = 0", [hoje]) || { total: 0 };
    res.json({
...
app.get('/api/logs/api-requests', requireAdmin, (req, res) => {
    ...
    if (status === 'erro') { sql += ' AND status_code >= 400'; }
    else if (status === 'ok') { sql += ' AND status_code < 400'; }
    else if (status) { sql += ' AND status_code = ?'; params.push(Number(status)); }
    if (data_inicio) { sql += ' AND criado_em >= ?'; params.push(data_inicio); }
    if (data_fim) { sql += ' AND criado_em <= ?'; params.push(data_fim + ' 23:59:59'); }
    sql += ' ORDER BY criado_em DESC';
    sql += ` LIMIT ${Math.min(Math.max(parseInt(lim, 10) || 100, 1), 500)} OFFSET ${Math.max(parseInt(off, 10) || 0, 0)}`;
...
app.get('/api/logs/webhooks', requireAdmin, (req, res) => {
    ...
    if (sucesso === '1') { sql += ' AND sucesso = 1'; }
    else if (sucesso === '0') { sql += ' AND sucesso = 0'; }
    if (data_inicio) { sql += ' AND criado_em >= ?'; params.push(data_inicio); }
    if (data_fim) { sql += ' AND criado_em <= ?'; params.push(data_fim + ' 23:59:59'); }
    sql += ' ORDER BY criado_em DESC';
    sql += ` LIMIT ${Math.min(Math.max(parseInt(lim, 10) || 100, 1), 500)} OFFSET ${Math.max(parseInt(off, 10) || 0, 0)}`;
...
app.get('/api/logs/integracoes', requireAdmin, (req, res) => {
    ...
    let sql = "SELECT * FROM atividades_log WHERE modulo IN ('integracoes', 'configuracoes', 'erp', 'api', 'webhook', 'whatsapp')";
    const params = [];
    if (modulo) { sql += ' AND modulo = ?'; params.push(modulo); }
    if (data_inicio) { sql += ' AND criado_em >= ?'; params.push(data_inicio); }
    if (data_fim) { sql += ' AND criado_em <= ?'; params.push(data_fim + ' 23:59:59'); }
...
app.get('/api/logs/erp-comunicacao', requireAdmin, (req, res) => {
    ...
    let sql = 'SELECT id, erp_tipo, erp_label, direcao, metodo, url, response_status, tempo_resposta_ms, sucesso, erro, contexto, criado_em FROM erp_communication_log WHERE 1=1';
    const params = [];
    if (erp_tipo) { sql += ' AND erp_tipo = ?'; params.push(erp_tipo); }
    if (sucesso !== undefined && sucesso !== '') { sql += ' AND sucesso = ?'; params.push(Number(sucesso)); }
    if (contexto) { sql += ' AND contexto LIKE ?'; params.push(`%${contexto}%`); }
    if (data_inicio) { sql += ' AND criado_em >= ?'; params.push(data_inicio); }
    if (data_fim) { sql += ' AND criado_em <= ?'; params.push(data_fim + ' 23:59:59'); }
    sql += ' ORDER BY criado_em DESC';
    sql += ` LIMIT ${Math.min(Math.max(parseInt(lim, 10) || 100, 1), 500)} OFFSET ${Math.max(parseInt(off, 10) || 0, 0)}`;
...
app.get('/api/logs/erp-comunicacao/:id', requireAdmin, (req, res) => {
    ...
    try { log.request_headers = JSON.parse(log.request_headers); } catch {}
    try { log.response_headers = JSON.parse(log.response_headers); } catch {}
    try { log.response_body_parsed = JSON.parse(log.response_body); } catch {}
    res.json(log);
});
...
app.post('/api/config/geral/logo', upload.single('logo'), (req, res) => {
    ...
    db.queryRun("INSERT INTO config_geral (chave, valor) VALUES ('logo_url', ?) ON CONFLICT(chave) DO UPDATE SET valor = ?", [logo_url, logo_url]);
    res.json({ logo_url });
});
...
function criarNotificacao(usuario_id, tipo, titulo, mensagem, link) {
    ...
    } catch (e) { console.error('Erro ao criar notificacao:', e.message); }
}
...
function criarNotificacaoParaPerfil(perfil, tipo, titulo, mensagem, link) {
    try {
        ...
        for (const u of users) {
            db.queryRun('INSERT INTO notificacoes (usuario_id, tipo, titulo, mensagem, link) VALUES (?, ?, ?, ?, ?)',
                [u.id, tipo, titulo, mensagem || '', link || '']);
        }
    } catch (e) { console.error('Erro notificacao perfil:', e.message); }
}
...
function criarNotificacaoParaTodos(tipo, titulo, mensagem, link) {
    ...
    } catch (e) { console.error('Erro notificacao todos:', e.message); }
}
...
app.get('/api/notificacoes/contagem', (req, res) => {
    ...
    const nao_lidas = db.queryGet('SELECT COUNT(*) as c FROM notificacoes WHERE usuario_id = ? AND lida = 0', [req.session.usuario.id]);
    res.json({ total: total.c, nao_lidas: nao_lidas.c });
});

app.patch('/api/notificacoes/:id/lida', (req, res) => {
    const db = getDB();
    db.queryRun('UPDATE notificacoes SET lida = 1 WHERE id = ? AND usuario_id = ?', [Number(req.params.id), req.session.usuario.id]);
    res.json({ ok: true });
});
...
app.get('/api/comentarios/:tipo/:id', (req, res) => {
    ...
    res.json(db.queryAll('SELECT * FROM comentarios WHERE entidade_tipo = ? AND entidade_id = ? ORDER BY criado_em ASC',
        [tipo, Number(id)]));
});
...
app.post('/api/comentarios/:tipo/:id', (req, res) => {
    ...
    const result = db.queryRun('INSERT INTO comentarios (entidade_tipo, entidade_id, usuario_id, usuario_nome, texto) VALUES (?, ?, ?, ?, ?)',
    ...
    registrarAtividade(req, 'criar', tipo === 'chamado' ? 'chamados' : 'projetos', Number(id), `Comentario: ${texto.substring(0, 100)}`);
...
app.get('/api/busca', (req, res) => {
    const db = getDB();
    const q = req.query.q;
    if (!q || q.length < 2) return res.json({ chamados: [], provedores: [], projetos: [], treinamentos: [], vendas: [] });
...
app.post('/api/config/backup', requireAdmin, (req, res) => {
    try {
        ...
        db.queryRun('INSERT INTO backups_log (nome_arquivo, tamanho, tipo, criado_por) VALUES (?, ?, ?, ?)',
            [nomeArquivo, stats.size, 'manual', req.session.usuario.nome]);
...
app.post('/api/projetos/:id/anexos', upload.array('arquivos', 10), (req, res) => {
    ...
    const anexos = req.files.map(f => {
        ...
        return { id: result.lastInsertRowid, nome_arquivo: f.originalname, caminho: `/uploads/${f.filename}`, tipo_mime: f.mimetype, tamanho: f.size };
    });
...
app.patch('/api/chamados/:id/status', (req, res) => {
    ...
    const updates = status === 'resolvido' || status === 'fechado'
...
app.get('/api/dashboard/widgets', requireAuth, (req, res) => {
    const db = getDB();
    const userId = req.session.usuario.id;
    let widgets = db.queryAll('SELECT id, widget_tipo, posicao, largura, visivel, config FROM dashboard_widgets WHERE usuario_id = ? ORDER BY posicao', [userId]);
    if (widgets.length === 0) {
        // Seed defaults
        WIDGETS_PADRAO.forEach(w => {
            db.queryRun('INSERT INTO dashboard_widgets (usuario_id, widget_tipo, posicao, largura, visivel) VALUES (?, ?, ?, ?, 1)', [userId, w.widget_tipo, w.posicao, w.largura]);
        });
        widgets = db.queryAll('SELECT id, widget_tipo, posicao, largura, visivel, config FROM dashboard_widgets WHERE usuario_id = ? ORDER BY posicao', [userId]);
    }
    res.json(widgets);
});
...
app.put('/api/dashboard/widgets', requireAuth, (req, res) => {
    ...
    widgets.forEach((w, i) => {
        db.queryRun('UPDATE dashboard_widgets SET posicao = ?, largura = ?, visivel = ? WHERE id = ? AND usuario_id = ?',
            [w.posicao !== undefined ? w.posicao : i, w.largura || 6, w.visivel !== undefined ? w.visivel : 1, w.id, userId]);
    });
    res.json({ ok: true });
});
...
function pdfHeader(doc, titulo) {
    ...
    doc.moveDown(0.3);
    doc.fontSize(9).font('Helvetica').fillColor('#666').text(`Gerado em: ${new Date().toLocaleString('pt-BR')}`, { align: 'center' });
...
function pdfTableRow(doc, cols, widths, isHeader) {
    ...
    doc.y = y + 14;
    if (isHeader) { doc.moveTo(50, doc.y).lineTo(545, doc.y).stroke('#ddd'); doc.moveDown(0.2); }
}
...
app.get('/api/relatorios/chamados/pdf', (req, res) => {
    ...
    if (provedor_id) { sql += ' AND c.provedor_id = ?'; params.push(Number(provedor_id)); }
    if (data_inicio) { sql += ' AND c.data_abertura >= ?'; params.push(data_inicio); }
    if (data_fim) { sql += ' AND c.data_abertura <= ?'; params.push(data_fim + ' 23:59:59'); }
    ...
    chamados.forEach(c => {
        ...
        pdfTableRow(doc, [c.id, c.provedor_nome, c.titulo, c.categoria, c.status, (c.data_abertura || '').substring(0, 10)], widths);
    });
    doc.end();
});
...
app.get('/api/relatorios/vendas/pdf', (req, res) => {
    const db = getDB();
    const negocios = db.queryAll('SELECT n.*, p.nome as provedor_nome FROM vendas_negocios n LEFT JOIN provedores p ON n.provedor_id = p.id ORDER BY n.atualizado_em DESC');
    ...
    negocios.forEach(n => {
        if (doc.y > 750) doc.addPage();
        pdfTableRow(doc, [n.provedor_nome_lead || n.provedor_nome || '-', n.responsavel_vendedor || '-', n.estagio, 'R$ ' + (n.valor_estimado || 0), n.origem || '-', (n.atualizado_em || '').substring(0, 10)], widths);
    });
    doc.end();
});
...
app.get('/api/relatorios/treinamentos/pdf', (req, res) => {
    const db = getDB();
    const treinamentos = db.queryAll("SELECT t.*, COALESCE(t.status, 'agendado') as status, p.nome as provedor_nome FROM treinamentos t JOIN provedores p ON t.provedor_id = p.id ORDER BY t.data_treinamento DESC");
    ...
    treinamentos.forEach(t => {
        if (doc.y > 750) doc.addPage();
        pdfTableRow(doc, [t.id, t.provedor_nome, t.titulo, t.status, (t.data_treinamento || '').substring(0, 10), t.hora_treinamento || '-'], widths);
    });
    doc.end();
});
...
app.post('/api/usuarios', requireAdmin, (req, res) => {
    ...
    if (!/^[a-zA-Z0-9._-]+$/.test(usuario)) return res.status(400).json({ erro: 'Usuario deve conter apenas letras, numeros, pontos, hifens e underscores' });
    erro = validarSenha(senha);
    if (erro) return res.status(400).json({ erro });
    if (!['admin', 'analista', 'vendedor', 'gestor_atendimento', 'gerente_noc', 'financeiro', 'atendente'].includes(perfil)) return res.status(400).json({ erro: 'Perfil inválido' });

    try {
        const senhaHash = bcrypt.hashSync(senha, 12);
        const result = db.queryRun('INSERT INTO usuarios (nome, usuario, senha, perfil) VALUES (?, ?, ?, ?)',
            [sanitizarTexto(nome), sanitizarTexto(usuario), senhaHash, perfil || 'analista']);
        const novo = db.queryGet('SELECT id, nome, usuario, perfil, ativo, criado_em FROM usuarios WHERE id = ?', [result.lastInsertRowid]);
        registrarAtividade(req, 'criar', 'usuarios', novo.id, `Usuario criado: ${nome} (${perfil})`);
        res.status(201).json(novo);
    } catch (err) {
        if (err.message && err.message.includes('UNIQUE')) return res.status(400).json({ erro: 'Já existe um usuário com esse login' });
        handleError(res, err, 'operacao');
    }
});
    ...
    try {
        if (senha) {
            ...
            db.queryRun('UPDATE usuarios SET nome = ?, usuario = ?, senha = ?, perfil = ? WHERE id = ?',
                [nome, usuario, senhaHash, perfil, id]);
        } else {
...
app.patch('/api/usuarios/:id/ativo', requireAdmin, (req, res) => {
    ...
    if (req.session.usuario.id === id) return res.status(400).json({ erro: 'Você não pode desativar sua própria conta' });
...
app.get('/api/provedores/:id', (req, res) => {
    ...
    const totalChamados = db.queryGet('SELECT COUNT(*) as total FROM chamados WHERE provedor_id = ?', [Number(req.params.id)]).total;
    const totalTreinamentos = db.queryGet('SELECT COUNT(*) as total FROM treinamentos WHERE provedor_id = ?', [Number(req.params.id)]).total;
...
app.post('/api/provedores', requireModuleAccess('provedores'), (req, res) => {
    const db = getDB();
    const { nome, contato, observacoes, plano, adicionais, modelo_integracao, erp, responsavel, logo_url, token_integracao } = req.body;
    ...
    try {
        const result = db.queryRun(
            ...
            [nome, contato || null, observacoes || null, plano || null, adicionais || null, modelo_integracao || null, erp || null, responsavel || null, logo_url || null, token_integracao || null]
        );
        ...
        if (err.message && err.message.includes('UNIQUE')) return res.status(400).json({ erro: 'Já existe um provedor com esse nome' });
        handleError(res, err, 'operacao');
    }
});
...
app.put('/api/provedores/:id', requireModuleAccess('provedores'), (req, res) => {
    ...
    try {
        db.queryRun(
            ...
            [nome, contato || null, observacoes || null, plano || null, adicionais || null, modelo_integracao || null, erp || null, responsavel || null, logo_url || null, token_integracao || null, Number(req.params.id)]
        );
        const provedor = db.queryGet('SELECT * FROM provedores WHERE id = ?', [Number(req.params.id)]);
...
app.delete('/api/provedores/:id', requireModuleAccess('provedores'), (req, res) => {
    const db = getDB();
    const chamados = db.queryGet('SELECT COUNT(*) as total FROM chamados WHERE provedor_id = ?', [Number(req.params.id)]).total;
    if (chamados > 0) return res.status(400).json({ erro: 'Não é possível excluir: existem chamados vinculados a este provedor' });
...
app.get('/api/chamados', (req, res) => {
    ...
    let sql = 'SELECT c.*, p.nome as provedor_nome, u.nome as responsavel_nome FROM chamados c JOIN provedores p ON c.provedor_id = p.id LEFT JOIN usuarios u ON c.responsavel_id = u.id WHERE 1=1';
    const params = [];
...
app.get('/api/chamados/fila', requireAuth, (req, res) => {
    try {
        ...
        chamados.forEach(ch => {
            ...
            const scoreSLA = ch.sla_vencido ? (pesoSLA * 5) : (ch.horas_restantes_sla < 4 ? pesoSLA * 3 : ch.horas_restantes_sla < 12 ? pesoSLA * 1.5 : 0);
        ...
        const resumo = {
            ...
            tempo_medio_espera: chamados.length > 0 ? Math.round(chamados.reduce((s, c) => s + (c.dias_aberto || 0), 0) / chamados.length * 10) / 10 : 0
        };

        res.json({ chamados, resumo, config: { peso_prioridade: pesoPrioridade, peso_sla: pesoSLA, peso_tempo_espera: pesoTempo, peso_reaberturas: pesoReaberturas } });
    } catch (err) { handleError(res, err, 'Fila atendimento'); }
});

app.get('/api/chamados/:id', (req, res) => {
    const db = getDB();
    const chamado = db.queryGet('SELECT c.*, p.nome as provedor_nome, u.nome as responsavel_nome FROM chamados c JOIN provedores p ON c.provedor_id = p.id LEFT JOIN usuarios u ON c.responsavel_id = u.id WHERE c.id = ?', [Number(req.params.id)]);
...
app.post('/api/chamados', (req, res) => {
    ...
    let erro = validarInteiro(provedor_id, 'Provedor', 1) || validarString(titulo, 'Titulo', 3, 200) || validarString(categoria, 'Categoria', 1, 100);
    if (erro) return res.status(400).json({ erro });
    if (prioridade && !['baixa', 'normal', 'alta', 'critica'].includes(prioridade)) return res.status(400).json({ erro: 'Prioridade invalida' });
    if (descricao && descricao.length > 5000) return res.status(400).json({ erro: 'Descricao muito longa (max 5000 caracteres)' });
    ...
    const result = db.queryRun(
        ...
        [Number(provedor_id), titulo, descricao || null, categoria, responsavel_id ? Number(responsavel_id) : null, prio, sla_resposta_limite, sla_resolucao_limite]
    );
    const chamado = db.queryGet('SELECT c.*, p.nome as provedor_nome FROM chamados c JOIN provedores p ON c.provedor_id = p.id WHERE c.id = ?', [result.lastInsertRowid]);
    dispararNotificacao('chamado_aberto', { id: chamado.id, titulo, categoria, provedor: chamado.provedor_nome });
    dispararWebhooks('chamado.criado', { id: chamado.id, titulo, categoria, prioridade: prio, provedor: chamado.provedor_nome });
    registrarAtividade(req, 'criar', 'chamados', chamado.id, `Chamado criado: ${titulo} (${prio})`);
    criarNotificacaoParaPerfil('admin', 'chamado', 'Novo chamado aberto', `${titulo} - ${chamado.provedor_nome} [${prio.toUpperCase()}]`, '/chamados');
...
app.put('/api/chamados/:id', (req, res) => {
    ...
    if (req.session.usuario.perfil !== 'admin' && atual.responsavel_id && atual.responsavel_id !== req.session.usuario.id) {
    ...
    if (prioridade && prioridade !== atual.prioridade) {
        ...
        if (slaRegra) {
            ...
            sla_resposta_limite = new Date(abertura.getTime() + slaRegra.tempo_resposta_horas * 3600000).toISOString().replace('T', ' ').substring(0, 19);
            sla_resolucao_limite = new Date(abertura.getTime() + slaRegra.tempo_resolucao_horas * 3600000).toISOString().replace('T', ' ').substring(0, 19);
        }
    }

    // Verificar se SLA estourou
    const agora = new Date().toISOString().replace('T', ' ').substring(0, 19);
    const sla_estourado = (sla_resolucao_limite && agora > sla_resolucao_limite && status !== 'resolvido' && status !== 'fechado') ? 1 : 0;
    ...
    db.queryRun(
        ...
        [Number(provedor_id), titulo, descricao || null, categoria, status, resolucao || null, data_resolucao,
         responsavel_id ? Number(responsavel_id) : null, prio, sla_resposta_limite, sla_resolucao_limite, sla_respondido_em, sla_estourado, Number(req.params.id)]
    );

    const chamado = db.queryGet('SELECT c.*, p.nome as provedor_nome FROM chamados c JOIN provedores p ON c.provedor_id = p.id WHERE c.id = ?', [Number(req.params.id)]);
    if (status === 'resolvido' || status === 'fechado') {
...
app.post('/api/chamados/:id/anexos', upload.array('arquivos', 10), (req, res) => {
    ...
    for (const file of req.files) {
        ...
        db.queryRun('INSERT INTO anexos (chamado_id, nome_arquivo, caminho, tipo_mime, tamanho) VALUES (?, ?, ?, ?, ?)',
...
app.get('/api/treinamentos', (req, res) => {
    ...
    let sql = "SELECT t.*, COALESCE(t.status, 'agendado') as status, p.nome as provedor_nome FROM treinamentos t JOIN provedores p ON t.provedor_id = p.id";
    const params = [];
    if (provedor_id) { sql += ' WHERE t.provedor_id = ?'; params.push(Number(provedor_id)); }
...
app.post('/api/treinamentos', (req, res) => {
    ...
    if (!provedor_id || !titulo || !data_treinamento) return res.status(400).json({ erro: 'Provedor, título e data são obrigatórios' });

    const result = db.queryRun('INSERT INTO treinamentos (provedor_id, titulo, descricao, data_treinamento, status, hora_treinamento) VALUES (?, ?, ?, ?, ?, ?)',
        [Number(provedor_id), titulo, descricao || null, data_treinamento, status || 'agendado', hora_treinamento || null]);
    const treinamento = db.queryGet('SELECT t.*, p.nome as provedor_nome FROM treinamentos t JOIN provedores p ON t.provedor_id = p.id WHERE t.id = ?', [result.lastInsertRowid]);
    dispararNotificacao('treinamento_agendado', { titulo, provedor: treinamento.provedor_nome, data: data_treinamento, hora: hora_treinamento || '' });
    dispararWebhooks('treinamento.criado', { id: treinamento.id, titulo, provedor: treinamento.provedor_nome, data: data_treinamento });
...
app.put('/api/treinamentos/:id', (req, res) => {
    const db = getDB();
    const { provedor_id, titulo, descricao, data_treinamento, status, hora_treinamento } = req.body;
    db.queryRun('UPDATE treinamentos SET provedor_id = ?, titulo = ?, descricao = ?, data_treinamento = ?, status = ?, hora_treinamento = ? WHERE id = ?',
        [Number(provedor_id), titulo, descricao || null, data_treinamento, status || 'agendado', hora_treinamento || null, Number(req.params.id)]);
    const treinamento = db.queryGet('SELECT t.*, p.nome as provedor_nome FROM treinamentos t JOIN provedores p ON t.provedor_id = p.id WHERE t.id = ?', [Number(req.params.id)]);
...
app.get('/api/projetos', (req, res) => {
    ...
    let sql = 'SELECT pr.*, p.nome as provedor_nome, u.nome as responsavel_nome FROM projetos pr LEFT JOIN provedores p ON pr.provedor_id = p.id LEFT JOIN usuarios u ON pr.responsavel_id = u.id';
    const params = [];
    if (status) { sql += ' WHERE pr.status = ?'; params.push(status); }
...
app.post('/api/projetos', (req, res) => {
    const db = getDB();
    const { titulo, descricao, provedor_id, provedor_manual, status, prioridade, data_inicio, data_previsao, responsavel_id } = req.body;
    if (!titulo || !data_inicio) return res.status(400).json({ erro: 'Título e data de início são obrigatórios' });

    const result = db.queryRun('INSERT INTO projetos (titulo, descricao, provedor_id, provedor_manual, status, prioridade, data_inicio, data_previsao, responsavel_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
        [titulo, descricao || null, provedor_id ? Number(provedor_id) : null, provedor_manual || null, status || 'em_andamento', prioridade || 'media', data_inicio, data_previsao || null, responsavel_id ? Number(responsavel_id) : null]);
    const projeto = db.queryGet('SELECT pr.*, p.nome as provedor_nome FROM projetos pr LEFT JOIN provedores p ON pr.provedor_id = p.id WHERE pr.id = ?', [result.lastInsertRowid]);
...
app.put('/api/projetos/:id', (req, res) => {
    const db = getDB();
    const { titulo, descricao, provedor_id, provedor_manual, status, prioridade, data_inicio, data_previsao, responsavel_id, percentual_conclusao } = req.body;
    db.queryRun('UPDATE projetos SET titulo = ?, descricao = ?, provedor_id = ?, provedor_manual = ?, status = ?, prioridade = ?, data_inicio = ?, data_previsao = ?, responsavel_id = ?, percentual_conclusao = ? WHERE id = ?',
        [titulo, descricao || null, provedor_id ? Number(provedor_id) : null, provedor_manual || null, status, prioridade, data_inicio, data_previsao || null, responsavel_id ? Number(responsavel_id) : null, percentual_conclusao !== undefined ? Number(percentual_conclusao) : 0, Number(req.params.id)]);
    const projeto = db.queryGet('SELECT pr.*, p.nome as provedor_nome FROM projetos pr LEFT JOIN provedores p ON pr.provedor_id = p.id WHERE pr.id = ?', [Number(req.params.id)]);
    const statusLabels = { em_andamento: 'Em andamento', pausado: 'Pausado', concluido: 'Concluído', cancelado: 'Cancelado' };
...
app.patch('/api/projetos/:id/status', (req, res) => {
    ...
    if (percentual_conclusao !== undefined) db.queryRun('UPDATE projetos SET percentual_conclusao = ? WHERE id = ?', [Number(percentual_conclusao), id]);

    const atualizado = db.queryGet('SELECT pr.*, p.nome as provedor_nome FROM projetos pr LEFT JOIN provedores p ON pr.provedor_id = p.id WHERE pr.id = ?', [id]);
...
app.get('/api/dashboard/resumo', (req, res) => {
    ...
    res.json({ ...chamados, projetos_ativos: projetos.total, total_provedores: totalProvedores, total_treinamentos: totalTreinamentos });
});

app.get('/api/dashboard/chamados-por-provedor', (req, res) => {
    const db = getDB();
    res.json(db.queryAll('SELECT p.nome, COUNT(c.id) as total FROM chamados c JOIN provedores p ON c.provedor_id = p.id GROUP BY p.id ORDER BY total DESC'));
});
...
app.get('/api/dashboard/chamados-por-mes', (req, res) => {
    const db = getDB();
    res.json(db.queryAll("SELECT strftime('%Y-%m', data_abertura) as mes, COUNT(*) as total FROM chamados WHERE data_abertura >= date('now', '-12 months') GROUP BY mes ORDER BY mes"));
});

app.get('/api/dashboard/chamados-recentes', (req, res) => {
    const db = getDB();
    res.json(db.queryAll('SELECT c.id, c.titulo, c.categoria, c.status, c.data_abertura, p.nome as provedor_nome FROM chamados c JOIN provedores p ON c.provedor_id = p.id ORDER BY c.data_abertura DESC LIMIT 10'));
});

app.get('/api/dashboard/chamados-abertos-por-provedor', (req, res) => {
    const db = getDB();
    res.json(db.queryAll("SELECT p.nome, c.categoria, COUNT(*) as total FROM chamados c JOIN provedores p ON c.provedor_id = p.id WHERE c.status IN ('pendente', 'em_andamento') GROUP BY p.id, c.categoria ORDER BY p.nome, c.categoria"));
});

// --- Métricas de Provedores ---
app.get('/api/dashboard/provedores-por-responsavel', (req, res) => {
    const db = getDB();
    res.json(db.queryAll("SELECT COALESCE(responsavel, 'Sem responsável') as responsavel, COUNT(*) as total FROM provedores GROUP BY responsavel ORDER BY total DESC"));
});

app.get('/api/dashboard/provedores-por-modelo', (req, res) => {
    const db = getDB();
    res.json(db.queryAll("SELECT COALESCE(modelo_integracao, 'Não definido') as modelo, COUNT(*) as total FROM provedores GROUP BY modelo_integracao ORDER BY total DESC"));
});

app.get('/api/dashboard/provedores-por-erp', (req, res) => {
    const db = getDB();
    res.json(db.queryAll("SELECT COALESCE(erp, 'Não definido') as erp, COUNT(*) as total FROM provedores GROUP BY erp ORDER BY total DESC"));
});

app.get('/api/dashboard/provedores-por-plano', (req, res) => {
    const db = getDB();
    res.json(db.queryAll("SELECT COALESCE(plano, 'Não definido') as plano, COUNT(*) as total FROM provedores GROUP BY plano ORDER BY total DESC"));
});

// --- Métricas de Treinamentos ---
app.get('/api/dashboard/treinamentos-por-status', (req, res) => {
    const db = getDB();
    res.json(db.queryAll("SELECT COALESCE(status, 'agendado') as status, COUNT(*) as total FROM treinamentos GROUP BY status ORDER BY total DESC"));
});

app.get('/api/dashboard/treinamentos-por-mes', (req, res) => {
    const db = getDB();
    res.json(db.queryAll("SELECT strftime('%Y-%m', data_treinamento) as mes, COUNT(*) as total FROM treinamentos GROUP BY mes ORDER BY mes"));
});
...
app.get('/api/dashboard/provedor/:id/metricas', (req, res) => {
    try {
        ...
        const chamados = db.queryAll('SELECT id, titulo, categoria, status, data_abertura, data_resolucao FROM chamados WHERE provedor_id = ? ORDER BY data_abertura DESC', [id]);
        const treinamentos = db.queryAll('SELECT id, titulo, status, data_treinamento FROM treinamentos WHERE provedor_id = ? ORDER BY data_treinamento DESC', [id]);
        const projetos = db.queryAll('SELECT id, titulo, status, prioridade, data_inicio, data_previsao FROM projetos WHERE provedor_id = ? ORDER BY data_inicio DESC', [id]);
        ...
        res.json({ provedor, resumo, chamados: chamados.slice(0, 20), treinamentos: treinamentos.slice(0, 10), projetos });
    } catch (err) {
...
app.get('/api/historico/:provedor_id', (req, res) => {
    ...
    const chamados = db.queryAll("SELECT id, titulo, categoria, status, data_abertura as data, 'chamado' as tipo FROM chamados WHERE provedor_id = ?", [Number(req.params.provedor_id)]);
    const treinamentos = db.queryAll("SELECT id, titulo, data_treinamento as data, 'treinamento' as tipo FROM treinamentos WHERE provedor_id = ?", [Number(req.params.provedor_id)]);
    const projetos = db.queryAll("SELECT id, titulo, status, prioridade, data_inicio as data, 'projeto' as tipo FROM projetos WHERE provedor_id = ?", [Number(req.params.provedor_id)]);

    const eventos = [...chamados, ...treinamentos, ...projetos].sort((a, b) => (b.data || '').localeCompare(a.data || ''));
...
app.get('/api/vendas/negocios', (req, res) => {
    ...
    const params = [];
    const conditions = [];
    if (vendedor) { conditions.push('n.responsavel_vendedor = ?'); params.push(vendedor); }
    if (estagio) { conditions.push('n.estagio = ?'); params.push(estagio); }
...
app.get('/api/vendas/negocios/:id', (req, res) => {
    const db = getDB();
    const negocio = db.queryGet(`SELECT n.*, p.nome as provedor_nome FROM vendas_negocios n LEFT JOIN provedores p ON n.provedor_id = p.id WHERE n.id = ?`, [Number(req.params.id)]);
    if (!negocio) return res.status(404).json({ erro: 'Negócio não encontrado' });
    const interacoes = db.queryAll('SELECT * FROM vendas_interacoes WHERE negocio_id = ? ORDER BY criado_em DESC', [Number(req.params.id)]);
    res.json({ ...negocio, interacoes });
});

app.post('/api/vendas/negocios', (req, res) => {
    const db = getDB();
    const { provedor_id, provedor_nome_lead, contato_lead, estagio, plano_interesse, valor_estimado, responsavel_vendedor, origem, observacoes } = req.body;
    const vendedor = responsavel_vendedor || req.session.usuario.nome;
    if (!provedor_nome_lead && !provedor_id) return res.status(400).json({ erro: 'Nome do lead ou provedor é obrigatório' });
    try {
        const result = db.queryRun(
            ...
            [provedor_id ? Number(provedor_id) : null, provedor_nome_lead || null, contato_lead || null, estagio || 'lead', plano_interesse || null, valor_estimado || 0, vendedor, origem || null, observacoes || null]
        );
        const negocio = db.queryGet('SELECT n.*, p.nome as provedor_nome FROM vendas_negocios n LEFT JOIN provedores p ON n.provedor_id = p.id WHERE n.id = ?', [result.lastInsertRowid]);
        registrarAtividade(req, 'criar', 'vendas', negocio.id, `Negocio criado: ${provedor_nome_lead || ''}`);
        res.status(201).json(negocio);
    } catch (err) { handleError(res, err, 'operacao'); }
});

app.put('/api/vendas/negocios/:id', (req, res) => {
    const db = getDB();
    const { provedor_id, provedor_nome_lead, contato_lead, estagio, plano_interesse, valor_estimado, responsavel_vendedor, origem, observacoes, motivo_perda } = req.body;
    try {
        db.queryRun(
            ...
            [provedor_id ? Number(provedor_id) : null, provedor_nome_lead || null, contato_lead || null, estagio || 'lead', plano_interesse || null, valor_estimado || 0, responsavel_vendedor || req.session.usuario.nome, origem || null, observacoes || null, motivo_perda || null, Number(req.params.id)]
        );
        const negocio = db.queryGet('SELECT n.*, p.nome as provedor_nome FROM vendas_negocios n LEFT JOIN provedores p ON n.provedor_id = p.id WHERE n.id = ?', [Number(req.params.id)]);
        registrarAtividade(req, 'editar', 'vendas', Number(req.params.id), `Negocio editado: ${provedor_nome_lead || ''}`);
        res.json(negocio);
    } catch (err) { handleError(res, err, 'operacao'); }
});
...
app.patch('/api/vendas/negocios/:id/estagio', (req, res) => {
    ...
    db.queryRun(`UPDATE vendas_negocios SET estagio = ?, motivo_perda = ?, atualizado_em = datetime('now','localtime') WHERE id = ?`,
...
app.post('/api/vendas/negocios/:id/interacoes', (req, res) => {
    ...
    const result = db.queryRun('INSERT INTO vendas_interacoes (negocio_id, tipo, descricao, criado_por) VALUES (?, ?, ?, ?)',
        [Number(req.params.id), tipo || 'nota', descricao, req.session.usuario.nome]);
    db.queryRun(`UPDATE vendas_negocios SET atualizado_em = datetime('now','localtime') WHERE id = ?`, [Number(req.params.id)]);
...
app.get('/api/vendas/metas', (req, res) => {
    ...
    const params = [];
    const conditions = [];
    if (vendedor) { conditions.push('vendedor = ?'); params.push(vendedor); }
    if (periodo) { conditions.push('periodo_referencia = ?'); params.push(periodo); }
    if (conditions.length) sql += ' WHERE ' + conditions.join(' AND ');
...
app.get('/api/vendas/metas/progresso', (req, res) => {
    ...
    const resultado = metas.map(meta => {
        ...
        const comissao = percentual >= 100 && meta.percentual_comissao > 0 ? (valorAtual * meta.percentual_comissao / 100) : 0;
...
app.post('/api/vendas/metas', requireAdmin, (req, res) => {
    ...
    if (!vendedor || !tipo_meta || !valor_alvo || !periodo_referencia) return res.status(400).json({ erro: 'Campos obrigatórios: vendedor, tipo_meta, valor_alvo, periodo_referencia' });
    const result = db.queryRun('INSERT INTO vendas_metas (vendedor, tipo_meta, valor_alvo, percentual_comissao, periodo_referencia) VALUES (?, ?, ?, ?, ?)',
...
app.put('/api/vendas/metas/:id', requireAdmin, (req, res) => {
    const db = getDB();
    const { vendedor, tipo_meta, valor_alvo, percentual_comissao, periodo_referencia } = req.body;
    db.queryRun('UPDATE vendas_metas SET vendedor = ?, tipo_meta = ?, valor_alvo = ?, percentual_comissao = ?, periodo_referencia = ? WHERE id = ?',
        [vendedor, tipo_meta, Number(valor_alvo), Number(percentual_comissao) || 0, periodo_referencia, Number(req.params.id)]);
...
app.get('/api/vendas/tarefas', (req, res) => {
    ...
    const params = [];
    const conditions = [];
    if (vendedor) { conditions.push('t.responsavel = ?'); params.push(vendedor); }
...
app.post('/api/vendas/tarefas', (req, res) => {
    ...
    const result = db.queryRun('INSERT INTO vendas_tarefas (titulo, descricao, provedor_id, negocio_id, tipo, data_hora, responsavel) VALUES (?, ?, ?, ?, ?, ?, ?)',
        [titulo, descricao || null, provedor_id ? Number(provedor_id) : null, negocio_id ? Number(negocio_id) : null, tipo || 'follow_up', data_hora, resp]);
    const tarefa = db.queryGet(`SELECT t.*, p.nome as provedor_nome FROM vendas_tarefas t LEFT JOIN provedores p ON t.provedor_id = p.id WHERE t.id = ?`, [result.lastInsertRowid]);
...
app.put('/api/vendas/tarefas/:id', (req, res) => {
    ...
    db.queryRun('UPDATE vendas_tarefas SET titulo = ?, descricao = ?, provedor_id = ?, negocio_id = ?, tipo = ?, data_hora = ?, status = ?, responsavel = ? WHERE id = ?',
        [titulo, descricao || null, provedor_id ? Number(provedor_id) : null, negocio_id ? Number(negocio_id) : null, tipo || 'follow_up', data_hora, status || 'pendente', responsavel || req.session.usuario.nome, Number(req.params.id)]);
    const tarefa = db.queryGet(`SELECT t.*, p.nome as provedor_nome FROM vendas_tarefas t LEFT JOIN provedores p ON t.provedor_id = p.id WHERE t.id = ?`, [Number(req.params.id)]);
    res.json(tarefa);
});
...
app.get('/api/vendas/visitas', (req, res) => {
    ...
    const params = [];
    const conditions = [];
    if (vendedor) { conditions.push('v.responsavel = ?'); params.push(vendedor); }
    if (provedor_id) { conditions.push('v.provedor_id = ?'); params.push(Number(provedor_id)); }
...
app.post('/api/vendas/visitas', (req, res) => {
    const db = getDB();
    const { provedor_id, negocio_id, data_visita, hora_visita, tipo_visita, status, endereco, observacoes, resultado, responsavel } = req.body;
    if (!provedor_id || !data_visita) return res.status(400).json({ erro: 'Provedor e data são obrigatórios' });
    const resp = responsavel || req.session.usuario.nome;
    const result = db.queryRun('INSERT INTO vendas_visitas (provedor_id, negocio_id, data_visita, hora_visita, tipo_visita, status, endereco, observacoes, resultado, responsavel) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
        [Number(provedor_id), negocio_id ? Number(negocio_id) : null, data_visita, hora_visita || null, tipo_visita || 'presencial', status || 'agendada', endereco || null, observacoes || null, resultado || null, resp]);
    const visita = db.queryGet(`SELECT v.*, p.nome as provedor_nome FROM vendas_visitas v JOIN provedores p ON v.provedor_id = p.id WHERE v.id = ?`, [result.lastInsertRowid]);
...
app.put('/api/vendas/visitas/:id', (req, res) => {
    const db = getDB();
    const { provedor_id, negocio_id, data_visita, hora_visita, tipo_visita, status, endereco, observacoes, resultado, responsavel } = req.body;
    db.queryRun('UPDATE vendas_visitas SET provedor_id = ?, negocio_id = ?, data_visita = ?, hora_visita = ?, tipo_visita = ?, status = ?, endereco = ?, observacoes = ?, resultado = ?, responsavel = ? WHERE id = ?',
        [Number(provedor_id), negocio_id ? Number(negocio_id) : null, data_visita, hora_visita || null, tipo_visita || 'presencial', status || 'agendada', endereco || null, observacoes || null, resultado || null, responsavel || req.session.usuario.nome, Number(req.params.id)]);
    const visita = db.queryGet(`SELECT v.*, p.nome as provedor_nome FROM vendas_visitas v JOIN provedores p ON v.provedor_id = p.id WHERE v.id = ?`, [Number(req.params.id)]);
    res.json(visita);
});
...
app.get('/api/vendas/dashboard-vendedor', (req, res) => {
    const db = getDB();
    const vendedor = req.session.usuario.perfil === 'vendedor' ? req.session.usuario.nome : (req.query.vendedor || req.session.usuario.nome);
    const mesAtual = new Date().toISOString().substring(0, 7);
    const mesInicio = mesAtual + '-01';
    const mesFim = mesAtual + '-31 23:59:59';

    const stats = {
        negocios_ativos: db.queryGet("SELECT COUNT(*) as total FROM vendas_negocios WHERE responsavel_vendedor = ? AND estagio NOT IN ('ativado','perdido')", [vendedor]).total,
        ativacoes_mes: db.queryGet("SELECT COUNT(*) as total FROM vendas_negocios WHERE responsavel_vendedor = ? AND estagio = 'ativado' AND atualizado_em >= ? AND atualizado_em <= ?", [vendedor, mesInicio, mesFim]).total,
        tarefas_pendentes: db.queryGet("SELECT COUNT(*) as total FROM vendas_tarefas WHERE responsavel = ? AND status = 'pendente'", [vendedor]).total,
        visitas_mes: db.queryGet("SELECT COUNT(*) as total FROM vendas_visitas WHERE responsavel = ? AND data_visita >= ? AND data_visita <= ?", [vendedor, mesInicio, mesAtual + '-31']).total
    };

    const valorPipeline = db.queryGet("SELECT COALESCE(SUM(valor_estimado),0) as total FROM vendas_negocios WHERE responsavel_vendedor = ? AND estagio NOT IN ('ativado','perdido')", [vendedor]).total;
    const totalNegocios = db.queryGet('SELECT COUNT(*) as total FROM vendas_negocios WHERE responsavel_vendedor = ?', [vendedor]).total;
    const totalAtivados = db.queryGet("SELECT COUNT(*) as total FROM vendas_negocios WHERE responsavel_vendedor = ? AND estagio = 'ativado'", [vendedor]).total;
    const taxaConversao = totalNegocios > 0 ? ((totalAtivados / totalNegocios) * 100).toFixed(1) : 0;

    const negociosPorEstagio = db.queryAll('SELECT estagio, COUNT(*) as total FROM vendas_negocios WHERE responsavel_vendedor = ? GROUP BY estagio', [vendedor]);
    const ativacoesPorMes = db.queryAll(`SELECT strftime('%Y-%m', atualizado_em) as mes, COUNT(*) as total FROM vendas_negocios WHERE responsavel_vendedor = ? AND estagio = 'ativado' AND atualizado_em >= date('now','-6 months') GROUP BY mes ORDER BY mes`, [vendedor]);
    const proximasTarefas = db.queryAll(`SELECT t.*, p.nome as provedor_nome FROM vendas_tarefas t LEFT JOIN provedores p ON t.provedor_id = p.id WHERE t.responsavel = ? AND t.status = 'pendente' ORDER BY t.data_hora ASC LIMIT 5`, [vendedor]);

    // Metas do mes com progresso
    const metas = db.queryAll('SELECT * FROM vendas_metas WHERE vendedor = ? AND periodo_referencia = ?', [vendedor, mesAtual]);
    metas.forEach(meta => {
        let valorAtual = 0;
        if (meta.tipo_meta === 'quantidade_ativacoes') {
            valorAtual = db.queryGet("SELECT COUNT(*) as total FROM vendas_negocios WHERE responsavel_vendedor = ? AND estagio = 'ativado' AND atualizado_em >= ? AND atualizado_em <= ?", [meta.vendedor, mesInicio, mesFim]).total;
        } else if (meta.tipo_meta === 'quantidade_upsells') {
            valorAtual = db.queryGet("SELECT COUNT(*) as total FROM vendas_negocios WHERE responsavel_vendedor = ? AND estagio = 'ativado' AND plano_interesse LIKE '%full%' AND atualizado_em >= ? AND atualizado_em <= ?", [meta.vendedor, mesInicio, mesFim]).total;
        } else if (meta.tipo_meta === 'valor_contratos') {
            valorAtual = db.queryGet("SELECT COALESCE(SUM(valor_estimado),0) as total FROM vendas_negocios WHERE responsavel_vendedor = ? AND estagio = 'ativado' AND atualizado_em >= ? AND atualizado_em <= ?", [meta.vendedor, mesInicio, mesFim]).total;
        }
        meta.valor_atual = valorAtual;
        meta.percentual_atingido = meta.valor_alvo > 0 ? (valorAtual / meta.valor_alvo) * 100 : 0;
        meta.comissao_calculada = meta.percentual_atingido >= 100 && meta.percentual_comissao > 0 ? (valorAtual * meta.percentual_comissao / 100) : 0;
    });
    ...
    const perdidosMes = db.queryGet("SELECT COUNT(*) as total FROM vendas_negocios WHERE responsavel_vendedor = ? AND estagio = 'perdido' AND atualizado_em >= ? AND atualizado_em <= ?", [vendedor, mesInicio, mesFim]).total;
    ...
    res.json({ stats, valor_pipeline: valorPipeline, taxa_conversao: taxaConversao, negocios_por_estagio: negociosPorEstagio, ativacoes_por_mes: ativacoesPorMes, proximas_tarefas: proximasTarefas, metas, performance });
});
...
app.get('/api/vendas/ranking', (req, res) => {
    ...
    // Add meta progress
    vendedores.forEach(v => {
        const meta = db.queryGet('SELECT valor_alvo FROM vendas_metas WHERE vendedor = ? AND tipo_meta = ? AND periodo_referencia = ?',
...
async function gerarPropostaPDF(proposta) {
    ...
    return new Promise((resolve, reject) => {
        ...
        const dataFormatada = proposta.criado_em ? new Date(proposta.criado_em).toLocaleDateString('pt-BR') : new Date().toLocaleDateString('pt-BR');
        ...
        // ========== PLANOS E SERVICOS ==========
        let planos = [];
        try { planos = JSON.parse(proposta.planos || '[]'); } catch {}
...
async function gerarContratoPDF(contrato) {
    ...
    return new Promise((resolve, reject) => {
        ...
        const marginR = 50;
        const contentW = pageW - marginL - marginR;
        const dataFormatada = contrato.criado_em ? new Date(contrato.criado_em).toLocaleDateString('pt-BR') : new Date().toLocaleDateString('pt-BR');
        ...
        if (contrato.valor_mensal || contrato.valor_total) {
            ...
            if (contrato.valor_mensal) {
                ...
                doc.text(`R$ ${Number(contrato.valor_mensal).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`, marginL + 15, valY + 23);
            }
            if (contrato.valor_total) {
                doc.font('Helvetica').fontSize(9).fillColor('#666');
                doc.text('Valor Total:', rightCol, valY + 10);
                doc.font('Helvetica-Bold').fontSize(14).fillColor('#198754');
                doc.text(`R$ ${Number(contrato.valor_total).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`, rightCol, valY + 23);
            }
            doc.y = valY + 60;
        }
        ...
        if (contrato.data_inicio || contrato.data_fim) {
            ...
            const vigencia = [];
            if (contrato.data_inicio) vigencia.push(`Inicio: ${new Date(contrato.data_inicio).toLocaleDateString('pt-BR')}`);
        ...
        if (contrato.assinatura_nome) {
            ...
            doc.text(`Assinado em: ${assData} | IP: ${contrato.assinatura_ip || 'N/A'}`, marginL + 80, doc.y + 30, { width: contentW - 160, align: 'center' });
        }
        ...
        doc.text(`Gerado em ${new Date().toLocaleString('pt-BR')} | Contrato #${contrato.id}`, marginL, bottomY + 18, { width: contentW, align: 'center' });
...
// Helper: enviar email
async function enviarEmailProposta(proposta, emailDestino, mensagemExtra, incluirFormulario, formularioUrl, incluirPDF = true) {
    const db = getDB();
    const config = db.queryGet('SELECT * FROM config_email WHERE ativo = 1');
    if (!config || !config.smtp_user) throw new Error('Email nao configurado. Configure em Vendas > Propostas > Config Email.');
    ...
    if (incluirPDF && proposta.pdf_caminho && fs.existsSync(proposta.pdf_caminho)) {
        attachments.push({ filename: `Proposta_${proposta.provedor_nome.replace(/\s+/g, '_')}.pdf`, path: proposta.pdf_caminho });
    }
...
app.get('/api/vendas/propostas', (req, res) => {
    ...
    const params = [];
    const conditions = [];
    if (vendedor) { conditions.push('criado_por = ?'); params.push(vendedor); }
...
app.post('/api/vendas/propostas', (req, res) => {
    const db = getDB();
    const { negocio_id, provedor_id, provedor_nome, titulo, planos, valor_total, condicoes, validade_dias, email_destino, whatsapp_destino } = req.body;
    ...
    const result = db.queryRun(
        `INSERT INTO vendas_propostas (negocio_id, provedor_id, provedor_nome, titulo, planos, valor_total, condicoes, validade_dias, email_destino, whatsapp_destino, criado_por)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [negocio_id || null, provedor_id || null, provedor_nome, titulo, typeof planos === 'string' ? planos : JSON.stringify(planos || []), valor_total || 0, condicoes || '', validade_dias || 30, email_destino || '', whatsapp_destino || '', req.session.usuario.nome]
    );
...
app.put('/api/vendas/propostas/:id', (req, res) => {
    const db = getDB();
    const { provedor_nome, titulo, planos, valor_total, condicoes, validade_dias, email_destino, whatsapp_destino, status } = req.body;
    db.queryRun(
        ...
        [provedor_nome, titulo, typeof planos === 'string' ? planos : JSON.stringify(planos || []), valor_total || 0, condicoes || '', validade_dias || 30, email_destino || '', whatsapp_destino || '', status || 'rascunho', req.params.id]
    );
...
app.delete('/api/vendas/propostas/:id', (req, res) => {
    const db = getDB();
    const p = db.queryGet('SELECT * FROM vendas_propostas WHERE id = ?', [req.params.id]);
    if (p && p.pdf_caminho && fs.existsSync(p.pdf_caminho)) { try { fs.unlinkSync(p.pdf_caminho); } catch {} }
...
app.post('/api/vendas/propostas/:id/gerar-pdf',async (req, res) => {
    try {
        ...
        db.queryRun('UPDATE vendas_propostas SET pdf_caminho = ?, pdf_token = ? WHERE id = ?', [filePath, pdfToken, proposta.id]);
...
app.get('/api/vendas/propostas/:id/download', (req, res) => {
    const db = getDB();
    const p = db.queryGet('SELECT * FROM vendas_propostas WHERE id = ?', [req.params.id]);
    if (!p || !p.pdf_caminho || !fs.existsSync(p.pdf_caminho)) return res.status(404).json({ erro: 'PDF nao encontrado' });
...
app.post('/api/vendas/propostas/:id/enviar-whatsapp',async (req, res) => {
    try {
        ...
        const mensagem = req.body.mensagem || `Olá ${proposta.provedor_nome}! Segue nossa proposta comercial: *${proposta.titulo}*`;
        const rText = await enviarMensagemWhatsApp(whatsappId, mensagem);
        if (rText.status >= 400) {
            console.error('Erro ao enviar texto WhatsApp:', rText.data);
            return res.status(500).json({ erro: 'Falha ao enviar mensagem: ' + (rText.data?.message || JSON.stringify(rText.data)) });
        }
        ...
        if (req.body.incluir_pdf !== false) {
            ...
            const rPdf = await enviarMensagemWhatsApp(whatsappId, `📄 *Proposta Comercial em PDF*\nBaixe aqui: ${pdfUrl}`);
            if (rPdf.status >= 400) {
        ...
        if (req.body.incluir_formulario) {
            ...
            if (!form) {
                ...
                db.queryRun('INSERT INTO formularios_cadastro (proposta_id, provedor_nome, token) VALUES (?, ?, ?)', [proposta.id, proposta.provedor_nome, token]);
                form = db.queryGet('SELECT * FROM formularios_cadastro WHERE proposta_id = ?', [proposta.id]);
            }
            const baseUrl = `${req.protocol}://${req.get('host')}`;
            const rForm = await enviarMensagemWhatsApp(whatsappId, `Para prosseguir com a ativacao, preencha o formulario de cadastro:\n${baseUrl}/formulario/${form.token}`);
        ...
        db.queryRun("UPDATE vendas_propostas SET status = 'enviada', enviada_via = CASE WHEN enviada_via IS NULL OR enviada_via = '' THEN 'whatsapp' WHEN enviada_via = 'email' THEN 'ambos' ELSE enviada_via END, whatsapp_destino = ?, atualizado_em = datetime('now','localtime') WHERE id = ?", [chatId, proposta.id]);
        res.json({ ok: true });
    } catch (err) {
...
app.post('/api/vendas/propostas/:id/enviar-email',async (req, res) => {
    try {
        ...
        if (incluirPDF && (!proposta.pdf_caminho || !fs.existsSync(proposta.pdf_caminho))) {
            ...
            db.queryRun('UPDATE vendas_propostas SET pdf_caminho = ? WHERE id = ?', [proposta.pdf_caminho, proposta.id]);
        }
        ...
        await enviarEmailProposta(proposta, emailDestino, req.body.mensagem, req.body.incluir_formulario, formularioUrl, incluirPDF);

        db.queryRun("UPDATE vendas_propostas SET status = 'enviada', enviada_via = CASE WHEN enviada_via IS NULL OR enviada_via = '' THEN 'email' WHEN enviada_via = 'whatsapp' THEN 'ambos' ELSE enviada_via END, email_destino = ?, atualizado_em = datetime('now','localtime') WHERE id = ?", [emailDestino, proposta.id]);
        res.json({ ok: true });
    } catch (err) {
...
app.get('/api/vendas/formularios', (req, res) => {
    ...
    const params = [];
    if (vendedor) { sql += ' WHERE p.criado_por = ?'; params.push(vendedor); }
...
app.put('/api/config/email', requireAdmin, (req, res) => {
    ...
    if (existing) {
        ...
        db.queryRun('UPDATE config_email SET smtp_host=?, smtp_port=?, smtp_user=?, smtp_pass=?, nome_remetente=?, ativo=? WHERE id=?',
            [smtp_host || 'smtp.gmail.com', smtp_port || 587, smtp_user || '', passToSave || '', nome_remetente || 'Nexus', ativo ? 1 : 0, existing.id]);
    } else {
        db.queryRun('INSERT INTO config_email (smtp_host, smtp_port, smtp_user, smtp_pass, nome_remetente, ativo) VALUES (?,?,?,?,?,?)',
            [smtp_host || 'smtp.gmail.com', smtp_port || 587, smtp_user || '', smtp_pass || '', nome_remetente || 'Nexus', ativo ? 1 : 0]);
    }
    res.json({ ok: true });
});
...
app.post('/api/config/email/testar', requireAdmin, async (req, res) => {
    try {
        const db = getDB();
        const config = db.queryGet('SELECT * FROM config_email LIMIT 1');
        if (!config || !config.smtp_user || !config.smtp_pass) return res.status(400).json({ erro: 'Configure email e senha primeiro' });

        const smtpPort2 = config.smtp_port || 587;
        const transporter = nodemailer.createTransport({
            host: config.smtp_host || 'smtp.gmail.com', port: smtpPort2, secure: smtpPort2 === 465,
            auth: { user: config.smtp_user, pass: config.smtp_pass },
            tls: { rejectUnauthorized: true, minVersion: 'TLSv1.2' }
        });
        await transporter.sendMail({
...
app.get('/api/vendas/dashboard', (req, res) => {
    ...
    // KPIs gerais
    const totalNegocios = db.queryGet(`SELECT COUNT(*) as total FROM vendas_negocios WHERE 1=1${vFilter}`, vParams).total;
    const totalAtivados = db.queryGet(`SELECT COUNT(*) as total FROM vendas_negocios WHERE estagio = 'ativado'${vFilter}`, vParams).total;
    const totalPerdidos = db.queryGet(`SELECT COUNT(*) as total FROM vendas_negocios WHERE estagio = 'perdido'${vFilter}`, vParams).total;
    const taxaConversao = totalNegocios > 0 ? ((totalAtivados / totalNegocios) * 100).toFixed(1) : 0;
    const valorPipeline = db.queryGet(`SELECT COALESCE(SUM(valor_estimado),0) as total FROM vendas_negocios WHERE estagio NOT IN ('ativado','perdido')${vFilter}`, vParams).total;

    // Ativacoes e perdas do mes
    const ativacoesMes = db.queryGet(`SELECT COUNT(*) as total FROM vendas_negocios WHERE estagio = 'ativado' AND atualizado_em >= ? AND atualizado_em <= ?${vFilter}`, [mesInicio, mesFim, ...vParams]).total;
    const perdasMes = db.queryGet(`SELECT COUNT(*) as total FROM vendas_negocios WHERE estagio = 'perdido' AND atualizado_em >= ? AND atualizado_em <= ?${vFilter}`, [mesInicio, mesFim, ...vParams]).total;

    // Propostas stats
    const totalPropostas = db.queryGet(`SELECT COUNT(*) as total FROM vendas_propostas WHERE 1=1${pFilter}`, vParams).total;
    const propostasEnviadas = db.queryGet(`SELECT COUNT(*) as total FROM vendas_propostas WHERE status = 'enviada'${pFilter}`, vParams).total;
    const propostasAceitas = db.queryGet(`SELECT COUNT(*) as total FROM vendas_propostas WHERE status = 'aceita'${pFilter}`, vParams).total;
    const propostasRecusadas = db.queryGet(`SELECT COUNT(*) as total FROM vendas_propostas WHERE status = 'recusada'${pFilter}`, vParams).total;

    // Funil de conversao (negocios por estagio)
    const funil = db.queryAll(`SELECT estagio, COUNT(*) as total, COALESCE(SUM(valor_estimado),0) as valor FROM vendas_negocios WHERE 1=1${vFilter} GROUP BY estagio`, vParams);

    // Ativacoes por mes (ultimos 6 meses)
    const ativacoesPorMes = db.queryAll(`SELECT strftime('%Y-%m', atualizado_em) as mes, COUNT(*) as total FROM vendas_negocios WHERE estagio = 'ativado' AND atualizado_em >= date('now','-6 months')${vFilter} GROUP BY mes ORDER BY mes`, vParams);

    // Ranking vendedores (top 10)
    const rankingVendedores = db.queryAll(`SELECT responsavel_vendedor as vendedor, COUNT(*) as total_negocios, SUM(CASE WHEN estagio = 'ativado' THEN 1 ELSE 0 END) as ativacoes, COALESCE(SUM(CASE WHEN estagio = 'ativado' THEN valor_estimado ELSE 0 END),0) as valor_ativado FROM vendas_negocios GROUP BY responsavel_vendedor ORDER BY ativacoes DESC LIMIT 10`);

    // Negocios por origem
    const porOrigem = db.queryAll(`SELECT COALESCE(origem, 'Sem origem') as origem, COUNT(*) as total FROM vendas_negocios WHERE 1=1${vFilter} GROUP BY origem ORDER BY total DESC`, vParams);

    // Visitas do mes
    const visitasMes = db.queryGet(`SELECT COUNT(*) as total FROM vendas_visitas WHERE data_visita >= ? AND data_visita <= ?${vendedor ? ' AND responsavel = ?' : ''}`, [mesInicio, mesAtual + '-31', ...vParams]).total;

    res.json({
        kpis: { totalNegocios, totalAtivados, totalPerdidos, taxaConversao, valorPipeline, ativacoesMes, perdasMes, visitasMes },
        propostas: { total: totalPropostas, enviadas: propostasEnviadas, aceitas: propostasAceitas, recusadas: propostasRecusadas },
        funil, ativacoesPorMes, rankingVendedores, porOrigem
    });
});
...
app.get('/api/vendas/historico-provedor/:provedor_id', (req, res) => {
    ...
    negocios.forEach(n => {
        eventos.push({ tipo: 'negocio', icone: 'bi-funnel', cor: '#0d6efd', titulo: `Negocio: ${n.provedor_nome_lead || 'Pipeline'}`, descricao: `Estagio: ${n.estagio} | Valor: R$ ${(n.valor_estimado||0).toFixed(2)}`, data: n.criado_em, responsavel: n.responsavel_vendedor, id: n.id });
    });

    // Interacoes dos negocios
    const negocioIds = negocios.map(n => n.id);
    if (negocioIds.length > 0) {
        const interacoes = db.queryAll(`SELECT i.*, n.provedor_nome_lead FROM vendas_interacoes i JOIN vendas_negocios n ON i.negocio_id = n.id WHERE n.provedor_id = ? ORDER BY i.criado_em DESC`, [pid]);
        interacoes.forEach(i => {
            eventos.push({ tipo: 'interacao', icone: 'bi-chat-dots', cor: '#6c757d', titulo: `${i.tipo}: ${i.descricao.substring(0, 80)}`, descricao: `Negocio: ${i.provedor_nome_lead || '#' + i.negocio_id}`, data: i.criado_em, responsavel: i.criado_por, id: i.id });
        });
    }

    // Propostas
    const propostas = db.queryAll('SELECT * FROM vendas_propostas WHERE provedor_id = ? OR provedor_nome = ? ORDER BY criado_em DESC', [pid, provedor.nome]);
    propostas.forEach(p => {
        eventos.push({ tipo: 'proposta', icone: 'bi-file-earmark-pdf', cor: '#D93B63', titulo: `Proposta: ${p.titulo}`, descricao: `Status: ${p.status} | Valor: R$ ${(p.valor_total||0).toFixed(2)}${p.enviada_via ? ' | Via: ' + p.enviada_via : ''}`, data: p.criado_em, responsavel: p.criado_por, id: p.id });
    });

    // Visitas
    const visitas = db.queryAll('SELECT * FROM vendas_visitas WHERE provedor_id = ? ORDER BY data_visita DESC', [pid]);
    visitas.forEach(v => {
        eventos.push({ tipo: 'visita', icone: 'bi-geo-alt', cor: '#198754', titulo: `Visita ${v.tipo_visita}: ${v.status}`, descricao: `${v.endereco || ''} ${v.resultado ? '| Resultado: ' + v.resultado : ''}`.trim(), data: v.data_visita + (v.hora_visita ? ' ' + v.hora_visita : ''), responsavel: v.responsavel, id: v.id });
    });

    // Tarefas
    const tarefas = db.queryAll('SELECT * FROM vendas_tarefas WHERE provedor_id = ? ORDER BY data_hora DESC', [pid]);
    tarefas.forEach(t => {
        eventos.push({ tipo: 'tarefa', icone: 'bi-check2-square', cor: '#ffc107', titulo: `Tarefa: ${t.titulo}`, descricao: `Tipo: ${t.tipo} | Status: ${t.status}`, data: t.data_hora, responsavel: t.responsavel, id: t.id });
    });

    // Formularios
    const formularios = db.queryAll('SELECT f.* FROM formularios_cadastro f JOIN vendas_propostas p ON f.proposta_id = p.id WHERE p.provedor_id = ? OR p.provedor_nome = ? ORDER BY f.criado_em DESC', [pid, provedor.nome]);
    formularios.forEach(f => {
        eventos.push({ tipo: 'formulario', icone: 'bi-file-text', cor: '#0dcaf0', titulo: `Formulario: ${f.status}`, descricao: f.status === 'preenchido' ? 'Preenchido em ' + (f.preenchido_em || '') : 'Pendente', data: f.criado_em, responsavel: null, id: f.id });
    });
...
app.post('/api/vendas/templates', (req, res) => {
    ...
    const result = db.queryRun('INSERT INTO vendas_templates_proposta (nome, planos, condicoes, validade_dias, criado_por) VALUES (?, ?, ?, ?, ?)',
        [nome, typeof planos === 'string' ? planos : JSON.stringify(planos || []), condicoes || '', validade_dias || 30, req.session.usuario.nome]);
...
app.put('/api/vendas/templates/:id', (req, res) => {
    ...
    db.queryRun('UPDATE vendas_templates_proposta SET nome=?, planos=?, condicoes=?, validade_dias=? WHERE id=?',
        [nome, typeof planos === 'string' ? planos : JSON.stringify(planos || []), condicoes || '', validade_dias || 30, Number(req.params.id)]);
...
app.get('/api/vendas/propostas/:id/rastreamento', (req, res) => {
    ...
    if (!proposta) return res.status(404).json({ erro: 'Proposta nao encontrada' });
    const views = db.queryAll('SELECT * FROM vendas_propostas_views WHERE proposta_id = ? ORDER BY visualizado_em DESC LIMIT 50', [req.params.id]);
...
app.post('/api/vendas/contratos', (req, res) => {
    const db = getDB();
    const { negocio_id, proposta_id, provedor_id, provedor_nome, numero_contrato, titulo, conteudo, valor_mensal, valor_total, data_inicio, data_fim } = req.body;
    if (!titulo || !provedor_nome) return res.status(400).json({ erro: 'Titulo e provedor obrigatorios' });
    const result = db.queryRun(`INSERT INTO vendas_contratos (negocio_id, proposta_id, provedor_id, provedor_nome, numero_contrato, titulo, conteudo, valor_mensal, valor_total, data_inicio, data_fim, responsavel)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [negocio_id || null, proposta_id || null, provedor_id || null, provedor_nome, numero_contrato || null, titulo, conteudo || '', valor_mensal || 0, valor_total || 0, data_inicio || null, data_fim || null, req.session.usuario.nome]);
    res.json({ id: result.lastInsertRowid, ok: true });
});

app.put('/api/vendas/contratos/:id', (req, res) => {
    const db = getDB();
    const { provedor_nome, numero_contrato, titulo, conteudo, valor_mensal, valor_total, data_inicio, data_fim, status } = req.body;
    db.queryRun(`UPDATE vendas_contratos SET provedor_nome=?, numero_contrato=?, titulo=?, conteudo=?, valor_mensal=?, valor_total=?, data_inicio=?, data_fim=?, status=? WHERE id=?`,
        [provedor_nome, numero_contrato, titulo, conteudo, valor_mensal, valor_total, data_inicio, data_fim, status, req.params.id]);
    res.json({ ok: true });
});
...
app.post('/api/vendas/contratos/:id/gerar-pdf', async (req, res) => {
    ...
    } catch (err) { handleError(res, err, 'Gerar PDF contrato'); }
});
...
app.post('/api/vendas/contratos/:id/enviar', (req, res) => {
    ...
    db.queryRun('UPDATE vendas_contratos SET assinatura_token = ?, status = ? WHERE id = ?', [token, 'enviado', contrato.id]);
...
app.post('/api/vendas/comissoes/regras', requireAdmin, (req, res) => {
    ...
    const result = db.queryRun('INSERT INTO vendas_comissoes_regras (vendedor, tipo, percentual, valor_fixo, plano_filtro) VALUES (?, ?, ?, ?, ?)',
...
app.put('/api/vendas/comissoes/regras/:id', requireAdmin, (req, res) => {
    ...
    db.queryRun('UPDATE vendas_comissoes_regras SET vendedor=?, tipo=?, percentual=?, valor_fixo=?, plano_filtro=?, ativo=? WHERE id=?',
...
app.get('/api/vendas/comissoes', (req, res) => {
    ...
    const conditions = [];
    if (vendedor) { conditions.push('vendedor = ?'); params.push(vendedor); }
    if (periodo) { conditions.push('periodo = ?'); params.push(periodo); }
    if (conditions.length) sql += ' WHERE ' + conditions.join(' AND ');
    sql += ' ORDER BY criado_em DESC';
    res.json(db.queryAll(sql, params));
});
...
app.post('/api/vendas/comissoes/calcular', requireAdmin, (req, res) => {
    ...
    const ativados = db.queryAll(`SELECT * FROM vendas_negocios WHERE estagio = 'ativado' AND atualizado_em >= ? AND atualizado_em <= ?`, [mesInicio, mesFim]);

    let comissoesGeradas = 0;

    for (const negocio of ativados) {
        // Verificar se ja existe comissao para este negocio neste periodo
        const existe = db.queryGet('SELECT id FROM vendas_comissoes WHERE negocio_id = ? AND periodo = ?', [negocio.id, periodo]);
        if (existe) continue;

        // Buscar regras do vendedor
        const regras = db.queryAll('SELECT * FROM vendas_comissoes_regras WHERE vendedor = ? AND ativo = 1', [negocio.responsavel_vendedor]);
        ...
        for (const regra of regras) {
            ...
            if (aplicar) {
                ...
                const valorComissao = regra.tipo === 'por_valor' ?
                    (valorBase * (regra.percentual / 100)) + (regra.valor_fixo || 0) :
                    regra.valor_fixo || (valorBase * (regra.percentual / 100));

                db.queryRun(`INSERT INTO vendas_comissoes (vendedor, negocio_id, descricao, valor_base, percentual, valor_comissao, periodo) VALUES (?, ?, ?, ?, ?, ?, ?)`,
                    [negocio.responsavel_vendedor, negocio.id, `Ativacao: ${negocio.provedor_nome || negocio.provedor_nome_lead || 'Negocio #' + negocio.id}`, valorBase, regra.percentual, valorComissao, periodo]);
                comissoesGeradas++;
            }
        }
    }
...
app.patch('/api/vendas/comissoes/:id/status', requireAdmin, (req, res) => {
    const db = getDB();
    db.queryRun('UPDATE vendas_comissoes SET status = ? WHERE id = ?', [req.body.status || 'pendente', Number(req.params.id)]);
...
app.post('/api/vendas/propostas/:id/duplicar', (req, res) => {
    ...
    const result = db.queryRun(
        `INSERT INTO vendas_propostas (negocio_id, provedor_id, provedor_nome, titulo, planos, valor_total, condicoes, validade_dias, status, email_destino, whatsapp_destino, criado_por)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'rascunho', ?, ?, ?)`,
        [original.negocio_id, original.provedor_id, original.provedor_nome, original.titulo + ' (copia)', original.planos, original.valor_total, original.condicoes, original.validade_dias, original.email_destino, original.whatsapp_destino, req.session.usuario.nome]
    );
...
app.put('/api/vendas/followup/config/:id', requireAdmin, (req, res) => {
    ...
    db.queryRun('UPDATE vendas_followup_config SET dias_apos = ?, ativo = ?, mensagem = ? WHERE id = ?',
        [dias_apos || 3, ativo ? 1 : 0, mensagem || '', Number(req.params.id)]);
...
app.get('/api/vendas/followup/alertas', (req, res) => {
    ...
    const cfgSemResposta = db.queryGet("SELECT * FROM vendas_followup_config WHERE tipo = 'proposta_sem_resposta' AND ativo = 1");
    if (cfgSemResposta) {
        ...
        const params = [dias];
        if (vendedor) { sql += ' AND criado_por = ?'; params.push(vendedor); }
        const propostas = db.queryAll(sql, params);
        propostas.forEach(p => {
            alertas.push({ tipo: 'proposta_sem_resposta', icone: 'bi-hourglass-split', cor: 'warning', titulo: `Proposta sem resposta: ${p.titulo}`, descricao: `Enviada para ${p.provedor_nome} ha mais de ${dias} dias`, proposta_id: p.id, data: p.atualizado_em });
        });
    }

    // 2. Propostas expirando
    const cfgExpirando = db.queryGet("SELECT * FROM vendas_followup_config WHERE tipo = 'proposta_expirando' AND ativo = 1");
    if (cfgExpirando) {
        ...
        const propostas = db.queryAll(sql, params);
        propostas.forEach(p => {
            alertas.push({ tipo: 'proposta_expirando', icone: 'bi-clock-history', cor: 'danger', titulo: `Proposta expirando: ${p.titulo}`, descricao: `Validade: ${p.validade_dias} dias - Provedor: ${p.provedor_nome}`, proposta_id: p.id, data: p.criado_em });
        });
    }

    // 3. Formularios recentemente preenchidos
    const cfgForm = db.queryGet("SELECT * FROM vendas_followup_config WHERE tipo = 'formulario_preenchido' AND ativo = 1");
    if (cfgForm) {
        ...
        const params = [];
        if (vendedor) { sql += ' AND p.criado_por = ?'; params.push(vendedor); }
        const forms = db.queryAll(sql, params);
        forms.forEach(f => {
            alertas.push({ tipo: 'formulario_preenchido', icone: 'bi-file-check', cor: 'success', titulo: `Formulario preenchido: ${f.provedor_nome}`, descricao: `Preenchido em ${f.preenchido_em || ''}`, formulario_id: f.id, data: f.preenchido_em });
        });
    }
    ...
    if (cfgParado) {
        ...
        const params = [dias];
        if (vendedor) { sql += ' AND n.responsavel_vendedor = ?'; params.push(vendedor); }
        sql += ' ORDER BY dias_parado DESC';
        const negocios = db.queryAll(sql, params);
        negocios.forEach(n => {
            alertas.push({ tipo: 'negocio_parado', icone: 'bi-pause-circle', cor: 'danger',
                ...
                negocio_id: n.id, data: n.atualizado_em });
        });
    }

    // 5. Negocios sem nenhuma interacao
    const cfgSemAtiv = db.queryGet("SELECT * FROM vendas_followup_config WHERE tipo = 'negocio_sem_atividade' AND ativo = 1");
    if (cfgSemAtiv) {
        ...
        negocios.forEach(n => {
            ...
            if (diasSem >= dias) {
                alertas.push({ tipo: 'negocio_sem_atividade', icone: 'bi-exclamation-diamond', cor: 'warning',
                    ...
                    negocio_id: n.id, data: ultimaData });
            }
        });
    }
...
app.put('/api/fila-atendimento/config', requireAdmin, (req, res) => {
    ...
    if (existe) {
        db.queryRun('UPDATE fila_atendimento_config SET peso_prioridade = ?, peso_sla = ?, peso_tempo_espera = ?, peso_reaberturas = ? WHERE id = ?',
            [peso_prioridade || 3, peso_sla || 5, peso_tempo_espera || 2, peso_reaberturas || 1, existe.id]);
    } else {
        db.queryRun('INSERT INTO fila_atendimento_config (peso_prioridade, peso_sla, peso_tempo_espera, peso_reaberturas) VALUES (?, ?, ?, ?)',
...
app.post('/api/chamados/:id/assumir', requireAuth, (req, res) => {
    ...
    db.queryRun('UPDATE chamados SET responsavel_id = ?, status = ? WHERE id = ?',
        [req.session.usuario.id, chamado.status === 'pendente' ? 'em_andamento' : chamado.status, Number(req.params.id)]);
    registrarAtividade(req, 'assumir', 'chamados', Number(req.params.id), `Chamado assumido por ${req.session.usuario.nome}`);
    res.json(db.queryGet('SELECT c.*, p.nome as provedor_nome FROM chamados c JOIN provedores p ON c.provedor_id = p.id WHERE c.id = ?', [Number(req.params.id)]));
});
...
app.post('/api/nps/criar', requireAuth, (req, res) => {
    try {
        ...
        const result = db.queryRun(
            'INSERT INTO nps_pesquisas (chamado_id, provedor_id, token) VALUES (?, ?, ?)',
            [Number(chamado_id), chamado.provedor_id, token]
        );
        registrarAtividade(req, 'criar', 'nps', result.lastInsertRowid, `Pesquisa NPS criada para chamado #${chamado_id}`);
        res.json({ id: result.lastInsertRowid, token, link: `/api/nps/responder/${token}` });
    } catch (err) { handleError(res, err, 'NPS criar'); }
});
...
app.get('/api/nps/responder/:token', (req, res) => {
    ...
    res.setHeader('Content-Security-Policy', `default-src 'self'; script-src 'nonce-${nonce}'; style-src 'self' https://cdn.jsdelivr.net 'unsafe-inline'; font-src 'self' https://cdn.jsdelivr.net data:; img-src 'self' data:;`);
    if (pesquisa.respondido) return res.send(`
...
app.post('/api/nps/responder/:token', (req, res) => {
    try {
        ...
        if (nota === undefined || nota === null || nota < 0 || nota > 10) return res.status(400).json({ erro: 'Nota invalida (0-10)' });
        ...
        db.queryRun('UPDATE nps_pesquisas SET nota = ?, comentario = ?, respondido = 1, respondido_em = datetime(\'now\',\'localtime\') WHERE token = ?',
            [Number(nota), comentario || null, req.params.token]);
        res.json({ sucesso: true });
    } catch (err) { handleError(res, err, 'NPS responder'); }
});
...
app.get('/api/nps/dashboard', requireAuth, (req, res) => {
    try {
        ...
        const params = [];
        if (periodo) { filtro += ' AND strftime(\'%Y-%m\', n.respondido_em) = ?'; params.push(periodo); }
        if (provedor_id) { filtro += ' AND n.provedor_id = ?'; params.push(Number(provedor_id)); }
        ...
        if (total === 0) return res.json({ score: null, total: 0, promotores: 0, neutros: 0, detratores: 0, respostas: [], distribuicao: [] });
        ...
        const distribuicao = Array.from({length: 11}, (_, i) => ({
            nota: i, quantidade: respostas.filter(r => r.nota === i).length
        }));

        // NPS por provedor
        const porProvedor = {};
        respostas.forEach(r => {
            if (!porProvedor[r.provedor_nome]) porProvedor[r.provedor_nome] = { total: 0, soma: 0, promotores: 0, detratores: 0 };
        ...
        const npsProvedor = Object.entries(porProvedor).map(([nome, d]) => ({
            provedor: nome, total: d.total, media: Math.round(d.soma / d.total * 10) / 10,
        ...
        const evolucao = Object.entries(porMes).sort().map(([mes, d]) => ({
            mes, score: Math.round(((d.promotores - d.detratores) / d.total) * 100), media: Math.round(d.soma / d.total * 10) / 10, total: d.total
        }));
        ...
        res.json({ score, media, total, promotores, neutros, detratores, distribuicao, nps_provedor: npsProvedor, evolucao, pendentes, respostas: respostas.slice(0, 50) });
    } catch (err) { handleError(res, err, 'NPS dashboard'); }
});
...
app.get('/api/nps', requireAuth, (req, res) => {
    try {
        ...
        `);
        res.json(respostas);
    } catch (err) { handleError(res, err, 'NPS listar'); }
});
...
app.post('/api/nps/enviar-massa', requireAdmin, (req, res) => {
    try {
        ...
        for (const ch of chamadosSemNPS) {
            const token = crypto.randomBytes(16).toString('hex');
            db.queryRun('INSERT INTO nps_pesquisas (chamado_id, provedor_id, token) VALUES (?, ?, ?)', [ch.id, ch.provedor_id, token]);
            criadas++;
        }
        res.json({ criadas, total_chamados: chamadosSemNPS.length });
    } catch (err) { handleError(res, err, 'NPS massa'); }
});
...
app.get('/api/whatsapp/status', async (req, res) => {
    ...
    } catch { res.status(503).json({ erro: 'WAHA nao disponivel', status: 'STOPPED' }); }
});
...
app.get('/api/whatsapp/qr', async (req, res) => {
    try {
        ...
        res.json(r.data || {});
    } catch { res.status(503).json({ erro: 'WAHA nao disponivel' }); }
});

app.post('/api/whatsapp/start', async (req, res) => {
    try {
        let r = await wahaFetch(`/api/sessions/${WAHA_SESSION}/start`, { method: 'POST', body: JSON.stringify({ name: WAHA_SESSION }) });
        // Se sessao ja existe (FAILED/etc), parar e reiniciar
        if (r.status === 422) {
            await wahaFetch(`/api/sessions/${WAHA_SESSION}/stop`, { method: 'POST' });
            await new Promise(ok => setTimeout(ok, 1000));
            r = await wahaFetch(`/api/sessions/${WAHA_SESSION}/start`, { method: 'POST', body: JSON.stringify({ name: WAHA_SESSION }) });
        }
...
app.post('/api/whatsapp/stop', async (req, res) => {
    ...
    } catch { res.status(503).json({ erro: 'Erro ao desconectar' }); }
});
...
app.get('/api/whatsapp/chats', async (req, res) => {
    try {
        ...
        if (cachedChats && (now - cachedChatsTime) < 15000) {
            ...
            const r = await wahaFetch(`/api/${WAHA_SESSION}/chats?limit=${limit}&sortBy=conversationTimestamp&sortOrder=desc`);
        ...
        const db = getDB();
        const atendimentos = db.queryAll("SELECT chat_id, id, agente_id, agente_nome, status, criado_em, atribuido_em FROM whatsapp_atendimentos WHERE status IN ('fila','em_atendimento')");
        const atendMap = {};
        atendimentos.forEach(a => { atendMap[a.chat_id] = a; });

        let chats = rawChats.map(c => {
            const atend = atendMap[c.id] || null;
            return {
                ...c,
                atendimento: atend ? { id: atend.id, agente_id: atend.agente_id, agente_nome: atend.agente_nome, status: atend.status, criado_em: atend.criado_em } : null
            };
        });
    ...
    try {
        ...
        const msgs = raw.map(m => {
            ...
            return {
                ...
                quotedMsg: m.quotedMsg ? { body: m.quotedMsg.body || '', participant: m.quotedMsg._data?.notifyName || '' } : null,
...
app.post('/api/whatsapp/send', async (req, res) => {
    try {
        ...
        if (req.session.usuario.perfil !== 'admin' && !chatId.includes('@g.us')) {
            const db = getDB();
            const atend = db.queryGet("SELECT id FROM whatsapp_atendimentos WHERE chat_id = ? AND status = 'em_atendimento' AND agente_id = ?", [chatId, req.session.usuario.id]);
        ...
        if (r.status < 400 && msgId) {
            salvarMensagemLocal({
                messageId: msgId, chatId, chatName: '',
                fromMe: true, body: text, type: 'chat', senderName: '',
...
app.post('/api/whatsapp/send-file', upload.single('file'), async (req, res) => {
    try {
        ...
        const r = await wahaFetch(endpoint, {
            method: 'POST',
            body: JSON.stringify({ session: WAHA_SESSION, chatId, file: { data: base64Data }, caption: caption || '', fileName: req.file.originalname })
        });
        ...
        if (r.status < 400 && msgId) {
            salvarMensagemLocal({
                messageId: msgId, chatId, chatName: '',
                fromMe: true, body: caption || '', type: isImage ? 'image' : 'document', senderName: '',
                filename: req.file.originalname,
                timestamp: Math.floor(Date.now() / 1000)
            });
        }
        res.status(r.status).json(r.data);
    } catch { res.status(503).json({ erro: 'Erro ao enviar arquivo' }); }
});
...
app.get('/api/whatsapp/media/:messageId', async (req, res) => {
    try {
        ...
        res.send(buffer);
    } catch { res.status(503).json({ erro: 'Erro ao baixar midia' }); }
});
...
app.post('/api/whatsapp/react', async (req, res) => {
    try {
        ...
        const r = await wahaFetch('/api/reaction', {
            method: 'POST', body: JSON.stringify({ session: WAHA_SESSION, chatId, messageId, reaction: reaction || '' })
        });
        res.status(r.status).json(r.data);
    } catch { res.status(503).json({ erro: 'Erro ao reagir' }); }
});
...
app.post('/api/whatsapp/seen', async (req, res) => {
    try {
        ...
        const r = await wahaFetch('/api/sendSeen', { method: 'POST', body: JSON.stringify({ session: WAHA_SESSION, chatId }) });
        if (cachedChats) {
            const chat = cachedChats.find(c => c.id === chatId);
            if (chat) chat.unreadCount = 0;
        }
        res.json(r.data || { ok: true });
    } catch { res.status(503).json({ erro: 'Erro ao marcar como lida' }); }
});
...
app.post('/api/whatsapp/typing', async (req, res) => {
    try {
        ...
        await wahaFetch(`/api/${WAHA_SESSION}/presence`, {
            method: 'POST', body: JSON.stringify({ chatId, presence: 'typing' })
        });
        res.json({ ok: true });
...
app.post('/api/whatsapp/send-mass', massSendLimiter, async (req, res) => {
    try {
        ...
        if (!chatIds || !Array.isArray(chatIds) || !text) return res.status(400).json({ erro: 'chatIds e text obrigatorios' });
        const delay = Math.max(delayMs || 3000, 2000);
        const results = [];
        for (let i = 0; i < chatIds.length; i++) {
            try { await enviarMensagemWhatsApp(chatIds[i], text); results.push({ chatId: chatIds[i], status: 'sent' }); }
            catch (e) { results.push({ chatId: chatIds[i], status: 'error', erro: e.message }); }
            if (i < chatIds.length - 1) await new Promise(r => setTimeout(r, delay));
        }
        res.json({ total: chatIds.length, results });
    } catch { res.status(503).json({ erro: 'Erro envio em massa' }); }
});
...
app.post('/api/whatsapp/templates', (req, res) => {
    const db = getDB(); const { nome, texto, categoria } = req.body;
    if (!nome || !texto) return res.status(400).json({ erro: 'Nome e texto obrigatórios' });
    const r = db.queryRun('INSERT INTO whatsapp_templates (nome, texto, categoria) VALUES (?, ?, ?)', [nome, texto, categoria || 'geral']);
...
app.put('/api/whatsapp/templates/:id', (req, res) => {
    const db = getDB(); const { nome, texto, categoria } = req.body;
    db.queryRun('UPDATE whatsapp_templates SET nome = ?, texto = ?, categoria = ? WHERE id = ?', [nome, texto, categoria || 'geral', Number(req.params.id)]);
...
app.delete('/api/whatsapp/templates/:id', (req, res) => {
    getDB().queryRun('DELETE FROM whatsapp_templates WHERE id = ?', [Number(req.params.id)]); res.json({ sucesso: true });
});
...
app.post('/api/whatsapp/auto-respostas', (req, res) => {
    const db = getDB(); const { palavra_chave, resposta } = req.body;
    if (!palavra_chave || !resposta) return res.status(400).json({ erro: 'Palavra-chave e resposta obrigatórias' });
    const r = db.queryRun('INSERT INTO whatsapp_auto_respostas (palavra_chave, resposta) VALUES (?, ?)', [palavra_chave, resposta]);
...
app.put('/api/whatsapp/auto-respostas/:id', (req, res) => {
    const db = getDB(); const { palavra_chave, resposta, ativo } = req.body;
    db.queryRun('UPDATE whatsapp_auto_respostas SET palavra_chave = ?, resposta = ?, ativo = ? WHERE id = ?',
        [palavra_chave, resposta, ativo ? 1 : 0, Number(req.params.id)]);
...
app.delete('/api/whatsapp/auto-respostas/:id', (req, res) => {
    getDB().queryRun('DELETE FROM whatsapp_auto_respostas WHERE id = ?', [Number(req.params.id)]); res.json({ sucesso: true });
});

// --- Flow Builder CRUD ---
app.get('/api/whatsapp/flows', (req, res) => {
    const db = getDB();
    const flows = db.queryAll('SELECT id, nome, descricao, ativo, versao, criado_por, criado_em, atualizado_em FROM whatsapp_flows ORDER BY atualizado_em DESC');
    for (const f of flows) {
        const c = db.queryGet('SELECT COUNT(*) as total FROM whatsapp_flow_sessoes WHERE flow_id = ? AND status = ?', [f.id, 'ativo']);
...
app.post('/api/whatsapp/flows', (req, res) => {
    ...
    const r = db.queryRun('INSERT INTO whatsapp_flows (nome, descricao, dados_flow, criado_por) VALUES (?, ?, ?, ?)',
        [nome, descricao || '', dados_flow || '{}', req.session.usuario.nome]);
...
app.put('/api/whatsapp/flows/:id', (req, res) => {
    ...
    db.queryRun("UPDATE whatsapp_flows SET nome = ?, descricao = ?, dados_flow = ?, versao = ?, atualizado_em = datetime('now','localtime') WHERE id = ?",
...
app.put('/api/whatsapp/flows/:id/ativar', (req, res) => {
    ...
    db.queryRun("UPDATE whatsapp_flows SET ativo = ?, atualizado_em = datetime('now','localtime') WHERE id = ?", [ativo ? 1 : 0, id]);
    if (!ativo) db.queryRun("UPDATE whatsapp_flow_sessoes SET status = 'finalizado' WHERE flow_id = ? AND status = 'ativo'", [id]);
...
app.get('/api/whatsapp/flows/:id/sessoes', (req, res) => {
    res.json(getDB().queryAll('SELECT * FROM whatsapp_flow_sessoes WHERE flow_id = ? ORDER BY atualizado_em DESC LIMIT 100', [Number(req.params.id)]));
});
...
app.put('/api/whatsapp/notificacoes/:id', (req, res) => {
    const db = getDB(); const { ativo, chat_id, mensagem_template } = req.body;
    db.queryRun('UPDATE whatsapp_notificacoes SET ativo = ?, chat_id = ?, mensagem_template = ? WHERE id = ?',
        [ativo ? 1 : 0, chat_id || null, mensagem_template || null, Number(req.params.id)]);
...
app.get('/api/whatsapp/provedores-vinculados', (req, res) => {
    const db = getDB();
    res.json(db.queryAll('SELECT wp.*, p.nome as provedor_nome FROM whatsapp_provedores wp JOIN provedores p ON wp.provedor_id = p.id ORDER BY p.nome'));
});
...
app.post('/api/whatsapp/vincular-provedor', (req, res) => {
    ...
    try {
        ...
        if (existing) {
            db.queryRun('UPDATE whatsapp_provedores SET chat_id = ? WHERE provedor_id = ?', [chat_id, Number(provedor_id)]);
        } else {
            db.queryRun('INSERT INTO whatsapp_provedores (provedor_id, chat_id) VALUES (?, ?)', [Number(provedor_id), chat_id]);
        }
        res.json({ sucesso: true });
...
app.get('/api/whatsapp/provedor-por-chat/:chatId', (req, res) => {
    const db = getDB();
    const chatId = req.params.chatId;
    const v = db.queryGet('SELECT wp.*, p.nome as provedor_nome, p.id as provedor_id FROM whatsapp_provedores wp JOIN provedores p ON wp.provedor_id = p.id WHERE wp.chat_id = ?', [chatId]);
    res.json(v || null);
});
...
app.post('/api/whatsapp/agendamentos', (req, res) => {
    ...
    if (!chat_id || !texto || !data_envio) return res.status(400).json({ erro: 'chat_id, texto e data_envio obrigatórios' });
    const r = db.queryRun('INSERT INTO whatsapp_agendamentos (chat_id, chat_nome, texto, data_envio) VALUES (?, ?, ?, ?)',
...
app.get('/api/whatsapp/search', async (req, res) => {
    try {
        ...
        const results = rows.map(r => ({
            id: r.message_id, from: r.chat_id, fromMe: r.from_me === 1,
            body: r.body || '', type: r.type || 'chat', timestamp: r.timestamp || 0,
            senderName: r.sender_name || '', _chatId: r._chatId, _chatName: r._chatName || r._chatId
        }));
        res.json(results);
    } catch { res.status(503).json({ erro: 'Erro na busca' }); }
});
...
app.post('/api/whatsapp/forward', async (req, res) => {
    try {
        ...
        const r = await wahaFetch('/api/forwardMessage', {
            method: 'POST', body: JSON.stringify({ session: WAHA_SESSION, chatId, messageId })
        });
        res.status(r.status).json(r.data);
    } catch { res.status(503).json({ erro: 'Erro ao encaminhar' }); }
});
...
app.get('/api/whatsapp/messages-page/:chatId', async (req, res) => {
    try {
        ...
        const total = db.queryGet('SELECT COUNT(*) as cnt FROM whatsapp_mensagens WHERE chat_id = ?', [chatId])?.cnt || 0;
        const msgs = rows.map(r => ({
            id: r.message_id, from: r.chat_id, fromMe: r.from_me === 1,
            body: r.body || '', type: r.type || 'chat', timestamp: r.timestamp || 0,
            senderName: r.sender_name || '', hasMedia: !!r.media_url,
            _mediaUrl: r.media_url || null, filename: r.filename || null,
    ...
    } catch { res.status(503).json({ erro: 'Erro ao carregar mensagens' }); }
});
...
app.get('/api/whatsapp/export/:chatId', async (req, res) => {
    try {
        ...
        const msgs = rows.map(r => ({
            fromMe: r.from_me === 1, body: r.body || '', type: r.type || 'chat',
            timestamp: r.timestamp || 0, senderName: r.sender_name || ''
        }));
        ...
        res.end();
    } catch { res.status(503).json({ erro: 'Erro ao exportar' }); }
});

// --- Métricas WhatsApp ---
app.get('/api/whatsapp/metricas', (req, res) => {
    const db = getDB();
    const enviadas = db.queryGet("SELECT COUNT(*) as total FROM whatsapp_metricas WHERE tipo = 'enviada'") || { total: 0 };
    const recebidas = db.queryGet("SELECT COUNT(*) as total FROM whatsapp_metricas WHERE tipo = 'recebida'") || { total: 0 };
    const porDia = db.queryAll("SELECT date(timestamp) as dia, tipo, COUNT(*) as total FROM whatsapp_metricas WHERE timestamp >= date('now', '-30 days') GROUP BY dia, tipo ORDER BY dia");
    const topContatos = db.queryAll("SELECT chat_nome, COUNT(*) as total FROM whatsapp_metricas WHERE chat_nome IS NOT NULL GROUP BY chat_id ORDER BY total DESC LIMIT 10");
...
app.get('/api/whatsapp/atendimentos', (req, res) => {
    ...
    if (status === 'fila') { where += " AND a.status = 'fila'"; }
    else if (status === 'em_atendimento') { where += " AND a.status = 'em_atendimento'"; }
    else if (status === 'finalizado') { where += " AND a.status = 'finalizado'"; }
    else { where += " AND a.status IN ('fila','em_atendimento')"; }
...
app.post('/api/whatsapp/atendimentos/atribuir', (req, res) => {
    ...
    const atend = db.queryGet("SELECT * FROM whatsapp_atendimentos WHERE chat_id = ? AND status IN ('fila','em_atendimento')", [chat_id]);
    if (!atend) return res.status(404).json({ erro: 'Chat nao esta na fila' });
    if (atend.status === 'em_atendimento') return res.status(409).json({ erro: `Chat ja esta sendo atendido por ${atend.agente_nome || 'outro agente'}` });

    db.queryRun(`UPDATE whatsapp_atendimentos SET agente_id = ?, agente_nome = ?, status = 'em_atendimento',
        atribuido_em = datetime('now','localtime'),
        tempo_espera_seg = CAST((julianday('now','localtime') - julianday(criado_em)) * 86400 AS INTEGER)
        WHERE id = ?`, [user.id, user.nome, atend.id]);

    db.queryRun("INSERT INTO whatsapp_atendimentos_log (atendimento_id, acao, para_agente_id, usuario_nome) VALUES (?, 'atribuido', ?, ?)",
        [atend.id, user.id, user.nome]);

    const updated = db.queryGet("SELECT * FROM whatsapp_atendimentos WHERE id = ?", [atend.id]);
    broadcastSSE({ event: 'atendimento.atribuido', payload: { chat_id, agente_id: user.id, agente_nome: user.nome, status: 'em_atendimento', id: atend.id } });
    cachedChatsTime = 0;
...
app.post('/api/whatsapp/atendimentos/transferir', (req, res) => {
    ...
    const atend = db.queryGet("SELECT * FROM whatsapp_atendimentos WHERE chat_id = ? AND status = 'em_atendimento'", [chat_id]);
    if (!atend) return res.status(404).json({ erro: 'Atendimento nao encontrado' });
    if (user.perfil !== 'admin' && atend.agente_id !== user.id) return res.status(403).json({ erro: 'Voce nao esta atribuido a esta conversa' });
    ...
    db.queryRun("UPDATE whatsapp_atendimentos SET agente_id = ?, agente_nome = ?, atribuido_em = datetime('now','localtime') WHERE id = ?",
        [destino.id, destino.nome, atend.id]);

    db.queryRun("INSERT INTO whatsapp_atendimentos_log (atendimento_id, acao, de_agente_id, para_agente_id, usuario_nome) VALUES (?, 'transferido', ?, ?, ?)",
        [atend.id, deAgenteId, destino.id, user.nome]);

    broadcastSSE({ event: 'atendimento.transferido', payload: { chat_id, de_agente_id: deAgenteId, agente_id: destino.id, agente_nome: destino.nome, id: atend.id } });
    cachedChatsTime = 0;
    res.json({ ok: true });
});
...
app.post('/api/whatsapp/atendimentos/finalizar', (req, res) => {
    ...
    db.queryRun("UPDATE whatsapp_atendimentos SET status = 'finalizado', finalizado_em = datetime('now','localtime'), notas = ? WHERE id = ?",
        [notas || null, atend.id]);

    db.queryRun("INSERT INTO whatsapp_atendimentos_log (atendimento_id, acao, usuario_nome, detalhes) VALUES (?, 'finalizado', ?, ?)",
...
app.get('/api/whatsapp/atendimentos/metricas', (req, res) => {
    const db = getDB();
    const naFila = db.queryGet("SELECT COUNT(*) as total FROM whatsapp_atendimentos WHERE status = 'fila'") || { total: 0 };
    const emAtendimento = db.queryGet("SELECT COUNT(*) as total FROM whatsapp_atendimentos WHERE status = 'em_atendimento'") || { total: 0 };
    const finalizadosHoje = db.queryGet("SELECT COUNT(*) as total FROM whatsapp_atendimentos WHERE status = 'finalizado' AND date(finalizado_em) = date('now','localtime')") || { total: 0 };
    const tempoMedio = db.queryGet("SELECT AVG(tempo_espera_seg) as media FROM whatsapp_atendimentos WHERE status != 'fila' AND tempo_espera_seg > 0") || { media: 0 };
...
app.get('/api/whatsapp/atendimentos/agentes', (req, res) => {
    const db = getDB();
    const agentes = db.queryAll("SELECT id, nome, perfil FROM usuarios WHERE ativo = 1 AND perfil IN ('admin','analista','gestor_atendimento','gerente_noc','atendente') ORDER BY nome");
    res.json(agentes);
});
...
app.get('/api/ponto/status', (req, res) => {
    ...
    res.json({
        ...
        pausaAtiva,
        pausasHoje,
        config: config || { horario_entrada: '08:00', horario_saida: '18:00', almoco_inicio: '12:00', almoco_duracao_min: 60, home_office: 0, carga_horaria_min: 480 },
...
app.post('/api/ponto/registrar', (req, res) => {
    ...
    const estaNoAlmoco = registrosHoje.some(r => r.tipo === 'entrada_almoco') &&
        !registrosHoje.some(r => r.tipo === 'saida_almoco' && r.data_hora > registrosHoje.filter(r2 => r2.tipo === 'entrada_almoco').pop().data_hora);
    ...
    if (tipo === 'saida' && (!temEntrada || (ultimaSaida && ultimaEntrada && ultimaSaida.data_hora > ultimaEntrada.data_hora))) {
    ...
    const ip = req.headers['x-forwarded-for'] || req.connection.remoteAddress || '';
    const result = db.queryRun(
        "INSERT INTO ponto_registros (usuario_id, usuario_nome, tipo, ip) VALUES (?, ?, ?, ?)",
        [user.id, user.nome, tipo, ip]
    );
...
app.post('/api/ponto/pausar', (req, res) => {
    ...
    const result = db.queryRun(
        "INSERT INTO ponto_pausas (usuario_id, usuario_nome, motivo) VALUES (?, ?, ?)",
        [user.id, user.nome, motivo]
    );
...
app.post('/api/ponto/retomar', (req, res) => {
    ...
    const pausaAtiva = db.queryGet("SELECT * FROM ponto_pausas WHERE usuario_id = ? AND fim IS NULL ORDER BY inicio DESC LIMIT 1", [user.id]);
...
app.get('/api/ponto/equipe', requireGerenciaOuAdmin, (req, res) => {
    ...
    const equipe = usuarios.map(u => {
        ...
        const pausaAtiva = db.queryGet("SELECT motivo, inicio FROM ponto_pausas WHERE usuario_id = ? AND fim IS NULL", [u.id]);
        const pausasHoje = db.queryAll(
        ...
        if (ultimaEntrada) {
            ...
            else if (entradaAlmoco && (!saidaAlmoco || saidaAlmoco.data_hora < entradaAlmoco.data_hora)) estado = 'almoco';
        ...
        return {
            id: u.id, nome: u.nome, perfil: u.perfil, foto_url: u.foto_url,
            estado, registros, pausaAtiva, totalPausasMin, config
        };
    });
...
app.put('/api/ponto/config/:usuario_id', requireGerenciaOuAdmin, (req, res) => {
    ...
    const { horario_entrada, horario_saida, almoco_inicio, almoco_duracao_min, home_office, carga_horaria_min } = req.body;
    ...
    if (existe) {
        db.queryRun(
            ...
            [horario_entrada || '08:00', horario_saida || '18:00', almoco_inicio || '12:00', almoco_duracao_min || 60, home_office ? 1 : 0, carga_horaria_min || 480, userId]
        );
    } else {
        db.queryRun(
            ...
            [userId, horario_entrada || '08:00', horario_saida || '18:00', almoco_inicio || '12:00', almoco_duracao_min || 60, home_office ? 1 : 0, carga_horaria_min || 480]
        );
    }
...
app.get('/api/ponto/config/:usuario_id', requireGerenciaOuAdmin, (req, res) => {
    ...
    const userId = Number(req.params.usuario_id);
    const config = db.queryGet("SELECT * FROM ponto_config WHERE usuario_id = ?", [userId]);
    res.json(config || { horario_entrada: '08:00', horario_saida: '18:00', almoco_inicio: '12:00', almoco_duracao_min: 60, home_office: 0, carga_horaria_min: 480 });
});
...
app.post('/api/v1/ponto/registrar', (req, res) => {
    ...
    broadcastSSE({ event: `ponto.${tipo}`, payload: { usuario_id: usuario.id, usuario_nome: usuario.nome, tipo, origem: 'maquina_ponto' } });
    res.json({ ok: true, id: result.lastInsertRowid });
});
...
function processarAgendamentos() {
    try {
        ...
        const pendentes = db.queryAll("SELECT * FROM whatsapp_agendamentos WHERE status = 'pendente' AND data_envio <= ?", [agora]);
...
const ERP_ADAPTERS = {
    ixc: {
        ...
        normalizeClientes(items) {
            return items.map(c => ({
                id_externo: c.id, nome: c.razao || c.fantasia || c.nome || '',
                documento: c.cnpj_cpf || c.cpf || c.cnpj || '', email: c.email || '',
                telefone: c.telefone_celular || c.fone || '', endereco: c.endereco || '',
                status: c.ativo || '', _raw: c
            }));
        },
        normalizeContratos(items) {
            return items.map(c => ({
                id_externo: c.id, cliente_id_externo: c.id_cliente,
                plano: c.contrato || c.id_vd_servico || '', status: c.status || c.status_internet || '',
                valor: c.valor || 0, _raw: c
            }));
        },
        normalizePlanos(items) {
            return items.map(p => ({ id_externo: p.id, nome: p.nome || p.descricao || '', valor: p.valor || 0, _raw: p }));
        }
    },
    ispfy: {
        label: 'ISPFY',
        ...
        normalizeClientes(items) {
            return items.map(c => ({
                id_externo: c.id || c.codigo, nome: c.nome_razao || c.nome || '',
                documento: c.cpf_cnpj || c.documento || '', email: c.email || '',
                telefone: c.telefone || c.celular || '', endereco: c.endereco || '',
                status: c.status || '', _raw: c
            }));
        },
        normalizeContratos(items) {
            return items.map(c => ({
                id_externo: c.id || c.codigo, cliente_id_externo: c.id_cliente || c.cliente_id,
                plano: c.plano || c.nome_plano || '', status: c.status || '',
                valor: c.valor || 0, _raw: c
            }));
        },
        normalizePlanos(items) {
            return items.map(p => ({ id_externo: p.id || p.codigo, nome: p.nome || p.descricao || '', valor: p.valor || 0, _raw: p }));
        }
    },
    hubsoft: {
        label: 'Hubsoft',
        ...
        async getAccessToken(config) {
            ...
            const tokenRes = await fetch(`${config.url_base}/oauth/token`, {
                ...
                body: JSON.stringify({
                    grant_type: 'password', client_id: extras.client_id,
                    client_secret: extras.client_secret, username: extras.username, password: extras.password
                })
            });
        ...
        normalizeClientes(items) {
            return items.map(c => ({
                id_externo: c.id_cliente || c.codigo_cliente, nome: c.nome_razaosocial || '',
                documento: c.cpf_cnpj || '', email: c.email_principal || '',
                telefone: c.telefone_primario || '', endereco: c.endereco || '',
                status: c.status || '', _raw: c
            }));
        },
        normalizeContratos(items) {
            ...
            items.forEach(c => {
                if (c.servicos && Array.isArray(c.servicos)) {
                    c.servicos.forEach(s => contratos.push({
                        id_externo: s.id_cliente_servico, cliente_id_externo: c.id_cliente || c.codigo_cliente,
                        plano: s.nome || s.numero_plano || '', status: s.status || '',
                        valor: s.valor || 0, _raw: s
                    }));
                }
            });
            return contratos;
                ...
                if (c.servicos && Array.isArray(c.servicos)) {
                    c.servicos.forEach(s => {
                        if (s.nome && !planos.has(s.nome)) {
                            planos.set(s.nome, { id_externo: s.id_cliente_servico, nome: s.nome || '', valor: s.valor || 0, _raw: s });
                        }
                    });
                }
            });
    ...
    sgp: {
        ...
        buildHeaders(config) {
            ...
            if (extras.auth_mode === 'basic') {
                return { 'Authorization': `Basic ${Buffer.from(`${extras.basic_user}:${extras.basic_pass}`).toString('base64')}`, 'Content-Type': 'application/json' };
            }
        ...
        endpoints: { clientes: '/api/ura/consultacliente/', contratos: '/api/ura/consultacliente/', planos: '/api/ura/empresa/' },
        normalizeResponse(data) {
            return Array.isArray(data) ? data : (data.dados || data.data || data.registros || []);
        },
        normalizeClientes(items) {
            return items.map(c => ({
                id_externo: c.id || c.codigo, nome: c.nome || c.razao_social || '',
                documento: c.cpf_cnpj || c.documento || '', email: c.email || '',
                telefone: c.telefone || c.celular || '', endereco: c.endereco || '',
                status: c.status || '', _raw: c
            }));
        ...
        normalizeContratos(items) {
            return items.map(c => ({
                id_externo: c.id || c.codigo, cliente_id_externo: c.cliente_id || c.id_cliente,
                plano: c.plano || c.nome_plano || '', status: c.status || '',
                valor: c.valor || 0, _raw: c
            }));
        },
    ...
    atlaz: {
        ...
        normalizeClientes(items) {
            return items.map(c => ({
                id_externo: c.id, nome: c.name || c.razao_social || c.nome || '',
                documento: c.document || c.cpf_cnpj || '', email: c.email || '',
                telefone: c.phone || c.telefone || '', endereco: c.address || c.endereco || '',
                status: c.status || '', _raw: c
            }));
        },
        normalizeContratos(items) {
            return items.map(c => ({
                id_externo: c.id, cliente_id_externo: c.client_id || c.cliente_id,
                plano: c.plan || c.plano || '', status: c.status || '',
                valor: c.value || c.valor || 0, _raw: c
            }));
        },
        normalizePlanos(items) {
            return items.map(p => ({ id_externo: p.id, nome: p.name || p.nome || p.description || '', valor: p.value || p.valor || 0, _raw: p }));
        }
    }
};
...
async function erpFetch(config, endpoint, adapter, contexto) {
    ...
    if (adapter.buildBody) {
        ...
        if (body) { opts.method = opts.method === 'GET' ? 'POST' : opts.method; opts.body = JSON.stringify(body); }
    }
        ...
        try {
            ...
            db.queryRun(
                `INSERT INTO erp_communication_log (erp_tipo, erp_label, direcao, metodo, url, request_headers, request_body, response_status, response_headers, response_body, tempo_resposta_ms, sucesso, erro, contexto)
                 VALUES (?, ?, 'outbound', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                [
                    config.tipo, adapter.label, opts.method, url,
...
app.get('/api/erp/todos', requireAdmin, (req, res) => {
    ...
    const result = VALID_ERP_TYPES.map(tipo => {
        ...
        return {
            tipo, label: adapter.label, configurado: !!cfg,
            ativo: cfg ? cfg.ativo : 0, url_base: cfg ? cfg.url_base : null,
...
app.get('/api/erp/:tipo/config', requireAdmin, (req, res) => {
    ...
    const db = getDB();
    const adapter = ERP_ADAPTERS[tipo];
    const config = db.queryGet('SELECT id, tipo, url_base, extras, ativo, ultimo_sync, criado_em FROM config_erp WHERE tipo = ? ORDER BY id DESC LIMIT 1', [tipo]);
    if (!config) {
        return res.json({ tipo, url_base: '', extras: '{}', ativo: 0, label: adapter.label, authFields: adapter.authFields, extraFields: adapter.extraFields });
    }
    let extrasParsed = {};
    try { extrasParsed = JSON.parse(config.extras || '{}'); } catch {}
    ...
    res.json({ ...config, extras_parsed: extrasParsed, label: adapter.label, authFields: adapter.authFields, extraFields: adapter.extraFields });
});
...
app.post('/api/erp/:tipo/config', requireAdmin, (req, res) => {
    ...
    if (adapter.authFields.includes('token') && !token) return res.status(400).json({ erro: 'Token obrigatorio para ' + adapter.label });
    ...
    if (existing) {
        db.queryRun('UPDATE config_erp SET url_base = ?, token = ?, extras = ?, ativo = 1 WHERE id = ?', [url_base, token || null, finalExtras, existing.id]);
    } else {
        db.queryRun('INSERT INTO config_erp (tipo, url_base, token, extras) VALUES (?, ?, ?, ?)', [tipo, url_base, token || null, finalExtras]);
    }
    res.json({ ok: true });
});
...
app.get('/api/erp/:tipo/testar', requireAdmin, async (req, res) => {
    ...
    try {
        ...
        if (r.ok) {
            ...
            res.json({ ok: false, status: r.status, erro: `HTTP ${r.status}${body ? ': ' + body.substring(0, 200) : ''}` });
        }
...
app.get('/api/erp/:tipo/clientes', requireAdmin, async (req, res) => {
    ...
    } catch (err) { handleError(res, err, `${tipo} clientes`); }
});
...
app.get('/api/erp/:tipo/contratos', requireAdmin, async (req, res) => {
    ...
    } catch (err) { handleError(res, err, `${tipo} contratos`); }
});
...
app.get('/api/erp/:tipo/planos', requireAdmin, async (req, res) => {
    ...
    } catch (err) { handleError(res, err, `${tipo} planos`); }
});
...
app.post('/api/erp/:tipo/sync', requireAdmin, async (req, res) => {
    ...
    const startTime = Date.now();
    const db = getDB();
    let novos = 0, atualizados = 0, erros = 0, totalRegistros = 0;
    ...
    try {
        ...
        for (const cli of clientes) {
            try {
                ...
                if (existe) {
                    ...
                    db.queryRun('UPDATE provedores SET cnpj = COALESCE(?, cnpj), email = COALESCE(?, email), telefone = COALESCE(?, telefone), endereco = COALESCE(?, endereco), erp = ? WHERE id = ?',
                        [cli.documento || null, cli.email || null, cli.telefone || null, cli.endereco || null, tipo, existe.id]);
                    atualizados++;
                } else if (cli.nome) {
                    db.queryRun('INSERT INTO provedores (nome, contato, erp, cnpj, email, telefone, endereco) VALUES (?, ?, ?, ?, ?, ?, ?)',
                        [cli.nome, cli.telefone || cli.email || '', tipo, cli.documento || null, cli.email || null, cli.telefone || null, cli.endereco || null]);
                    novos++;
                }
            } catch { erros++; }
        }
        ...
        db.queryRun(
            ...
            [tipo, 'clientes', totalRegistros, novos, atualizados, erros, duracao, `Sync manual por ${req.session?.usuario?.nome || 'admin'}`]
        );

        registrarAtividade(req, 'sync', 'erp', null, `Sync ${tipo}: ${totalRegistros} registros, ${novos} novos, ${atualizados} existentes`);
...
app.get('/api/ixc/config', requireAdmin, (req, res) => {
    const db = getDB();
    let config = db.queryGet('SELECT id, url_base, ativo, ultimo_sync, criado_em FROM config_erp WHERE tipo = ? ORDER BY id DESC LIMIT 1', ['ixc']);
    if (!config) config = db.queryGet('SELECT id, url_base, ativo, ultimo_sync, criado_em FROM config_ixc ORDER BY id DESC LIMIT 1');
...
app.post('/api/ixc/config', requireAdmin, (req, res) => {
    ...
    if (existing) {
        db.queryRun('UPDATE config_erp SET url_base = ?, token = ?, ativo = 1 WHERE id = ?', [url_base, token, existing.id]);
    } else {
    ...
    if (existingOld) db.queryRun('UPDATE config_ixc SET url_base = ?, token = ?, ativo = 1 WHERE id = ?', [url_base, token, existingOld.id]);
...
app.get('/api/ixc/testar', requireAdmin, async (req, res) => {
    ...
    try {
        ...
        if (r.ok) {
            ...
            db.queryRun("UPDATE config_erp SET ultimo_sync = datetime('now','localtime') WHERE id = ?", [config.id]);
            res.json({ ok: true, status: r.status });
        } else { res.json({ ok: false, status: r.status, erro: `HTTP ${r.status}` }); }
    } catch (err) { res.json({ ok: false, erro: err.message }); }
});
...
app.get('/api/ixc/clientes', requireAdmin, async (req, res) => {
    ...
    } catch (err) { handleError(res, err, 'IXC clientes'); }
});
...
app.get('/api/ixc/contratos', requireAdmin, async (req, res) => {
    ...
    try {
        ...
        const data = await r.json();
        res.json(ERP_ADAPTERS.ixc.normalizeResponse(data));
    } catch (err) { handleError(res, err, 'IXC contratos'); }
});
...
app.get('/api/ixc/planos', requireAdmin, async (req, res) => {
    ...
    try {
        ...
        const data = await r.json();
        res.json(ERP_ADAPTERS.ixc.normalizeResponse(data));
    } catch (err) { handleError(res, err, 'IXC planos'); }
});
...
app.post('/api/kb/categorias', requireAuth, (req, res) => {
    ...
    if (!nome) return res.status(400).json({ erro: 'Nome obrigatorio' });
    const result = db.queryRun('INSERT INTO kb_categorias (nome, icone, cor) VALUES (?, ?, ?)', [nome, icone || 'bi-folder', cor || '#007bff']);
...
app.get('/api/kb/artigos', requireAuth, (req, res) => {
    ...
    const params = [];
    if (categoria_id) { sql += ' AND a.categoria_id = ?'; params.push(Number(categoria_id)); }
    if (busca) { sql += ' AND (a.titulo LIKE ? OR a.tags LIKE ? OR a.conteudo LIKE ?)'; params.push(`%${busca}%`, `%${busca}%`, `%${busca}%`); }
...
app.get('/api/kb/artigos/:id', requireAuth, (req, res) => {
    const db = getDB();
    const artigo = db.queryGet(`SELECT a.*, c.nome as categoria_nome, u.nome as autor_nome FROM kb_artigos a LEFT JOIN kb_categorias c ON a.categoria_id = c.id LEFT JOIN usuarios u ON a.autor_id = u.id WHERE a.id = ?`, [Number(req.params.id)]);
...
app.post('/api/kb/artigos', requireAuth, (req, res) => {
    ...
    const result = db.queryRun('INSERT INTO kb_artigos (titulo, conteudo, tags, categoria_id, autor_id, publicado) VALUES (?, ?, ?, ?, ?, ?)',
        [titulo, conteudo, tags || null, categoria_id ? Number(categoria_id) : null, autor_id, publicado !== undefined ? Number(publicado) : 1]);
...
app.put('/api/kb/artigos/:id', requireAuth, (req, res) => {
    const db = getDB();
    const { titulo, conteudo, tags, categoria_id, publicado } = req.body;
    db.queryRun(`UPDATE kb_artigos SET titulo = ?, conteudo = ?, tags = ?, categoria_id = ?, publicado = ?, atualizado_em = datetime('now','localtime') WHERE id = ?`,
        [titulo, conteudo, tags || null, categoria_id ? Number(categoria_id) : null, publicado !== undefined ? Number(publicado) : 1, Number(req.params.id)]);
...
app.get('/api/agenda/eventos', requireAuth, (req, res) => {
    ...
    const params = [];
    if (mes && ano) { sql += ` AND data_inicio LIKE ?`; params.push(`${ano}-${String(mes).padStart(2, '0')}%`); }
...
app.post('/api/agenda/eventos', requireAuth, (req, res) => {
    ...
    const usuario_id = req.session?.usuario?.id || null;
    const result = db.queryRun('INSERT INTO agenda_eventos (titulo, descricao, tipo, data_inicio, data_fim, cor, lembrete_minutos, usuario_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
        [titulo, descricao || null, tipo || 'evento', data_inicio, data_fim || null, cor || '#007bff', lembrete_minutos || null, usuario_id]);
...
app.put('/api/agenda/eventos/:id', requireAuth, (req, res) => {
    const db = getDB();
    const { titulo, descricao, tipo, data_inicio, data_fim, cor, lembrete_minutos } = req.body;
    db.queryRun('UPDATE agenda_eventos SET titulo = ?, descricao = ?, tipo = ?, data_inicio = ?, data_fim = ?, cor = ?, lembrete_minutos = ? WHERE id = ?',
        [titulo, descricao || null, tipo || 'evento', data_inicio, data_fim || null, cor || '#007bff', lembrete_minutos || null, Number(req.params.id)]);
...
app.get('/api/financeiro/faturas', requireAuth, (req, res) => {
    ...
    let sql = 'SELECT f.*, p.nome as provedor_nome FROM financeiro_faturas f JOIN provedores p ON f.provedor_id = p.id WHERE 1=1';
    const params = [];
    if (tipo) { sql += ' AND f.tipo = ?'; params.push(tipo); }
    if (status) { sql += ' AND f.status = ?'; params.push(status); }
    if (provedor_id) { sql += ' AND f.provedor_id = ?'; params.push(Number(provedor_id)); }
    if (mes) { sql += ' AND f.data_vencimento LIKE ?'; params.push(mes + '%'); }
...
app.get('/api/financeiro/faturas/:id', requireAuth, (req, res) => {
    const db = getDB();
    const f = db.queryGet('SELECT f.*, p.nome as provedor_nome FROM financeiro_faturas f JOIN provedores p ON f.provedor_id = p.id WHERE f.id = ?', [Number(req.params.id)]);
...
app.post('/api/financeiro/faturas', requireAuth, (req, res) => {
    ...
    if (!provedor_id || !valor || !data_vencimento) return res.status(400).json({ erro: 'Provedor, valor e vencimento obrigatorios' });
    const result = db.queryRun('INSERT INTO financeiro_faturas (provedor_id, descricao, valor, tipo, status, data_vencimento, forma_pagamento, observacoes) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
        [Number(provedor_id), descricao || null, Number(valor), tipo || 'receita', status || 'pendente', data_vencimento, forma_pagamento || null, observacoes || null]);
    registrarAtividade(req, 'criar', 'financeiro', result.lastInsertRowid, `Fatura R$${valor} (${tipo})`);
    res.status(201).json(db.queryGet('SELECT f.*, p.nome as provedor_nome FROM financeiro_faturas f JOIN provedores p ON f.provedor_id = p.id WHERE f.id = ?', [result.lastInsertRowid]));
});
...
app.put('/api/financeiro/faturas/:id', requireAuth, (req, res) => {
    ...
    if (status === 'pago') {
        const atual = db.queryGet('SELECT data_pagamento FROM financeiro_faturas WHERE id = ?', [Number(req.params.id)]);
        data_pagamento = atual?.data_pagamento || new Date().toISOString().split('T')[0];
    }
    db.queryRun('UPDATE financeiro_faturas SET provedor_id = ?, descricao = ?, valor = ?, tipo = ?, status = ?, data_vencimento = ?, data_pagamento = ?, forma_pagamento = ?, observacoes = ? WHERE id = ?',
        [Number(provedor_id), descricao || null, Number(valor), tipo, status, data_vencimento, data_pagamento, forma_pagamento || null, observacoes || null, Number(req.params.id)]);
    registrarAtividade(req, 'editar', 'financeiro', Number(req.params.id), `Fatura editada: R$${valor}`);
    res.json(db.queryGet('SELECT f.*, p.nome as provedor_nome FROM financeiro_faturas f JOIN provedores p ON f.provedor_id = p.id WHERE f.id = ?', [Number(req.params.id)]));
});

app.patch('/api/financeiro/faturas/:id/pagar', requireAuth, (req, res) => {
    const db = getDB();
    const hoje = new Date().toISOString().split('T')[0];
    db.queryRun("UPDATE financeiro_faturas SET status = 'pago', data_pagamento = ? WHERE id = ?", [hoje, Number(req.params.id)]);
...
app.get('/api/financeiro/resumo', requireAuth, (req, res) => {
    const db = getDB();
    const mesAtual = new Date().toISOString().substring(0, 7);
    const receitas = db.queryGet("SELECT COALESCE(SUM(valor), 0) as total FROM financeiro_faturas WHERE tipo = 'receita' AND status = 'pago' AND data_vencimento LIKE ?", [mesAtual + '%']);
    const despesas = db.queryGet("SELECT COALESCE(SUM(valor), 0) as total FROM financeiro_faturas WHERE tipo = 'despesa' AND status = 'pago' AND data_vencimento LIKE ?", [mesAtual + '%']);
    const inadimplencia = db.queryGet("SELECT COALESCE(SUM(valor), 0) as total FROM financeiro_faturas WHERE status IN ('pendente','vencido') AND data_vencimento < date('now','localtime')");
    res.json({ receitas: receitas.total, despesas: despesas.total, saldo: receitas.total - despesas.total, inadimplencia: inadimplencia.total });
});
...
app.get('/api/relatorios/chamados', requireAuth, (req, res) => {
    ...
    const tempoMedio = comResolucao.length > 0
        ? (comResolucao.reduce((acc, c) => acc + (new Date(c.data_resolucao) - new Date(c.data_abertura)) / 86400000, 0) / comResolucao.length).toFixed(1)
        : 0;
    ...
    res.json({ chamados, total, resolvidos, pendentes, tempo_medio_dias: tempoMedio, por_categoria: porCategoria, por_mes: porMes });
});
...
app.get('/api/relatorios/vendas', requireAuth, (req, res) => {
    ...
    const porEstagio = {};
    negocios.forEach(n => { porEstagio[n.estagio] = (porEstagio[n.estagio] || 0) + 1; });
...
app.get('/api/relatorios/treinamentos', requireAuth, (req, res) => {
    ...
    const params = [];
    if (data_inicio) { sql += ' AND t.data_treinamento >= ?'; params.push(data_inicio); }
    if (data_fim) { sql += ' AND t.data_treinamento <= ?'; params.push(data_fim); }
    if (provedor_id) { sql += ' AND t.provedor_id = ?'; params.push(Number(provedor_id)); }
    sql += ' ORDER BY t.data_treinamento DESC';
...
app.get('/api/relatorios/:tipo/csv', requireAuth, (req, res) => {
    ...
    if (tipo === 'chamados') {
        ...
        rows = db.queryAll('SELECT c.id, p.nome as provedor, c.titulo, c.categoria, c.prioridade, c.status, c.data_abertura, c.data_resolucao FROM chamados c JOIN provedores p ON c.provedor_id = p.id ORDER BY c.data_abertura DESC');
    } else if (tipo === 'vendas') {
        headers = ['ID', 'Lead', 'Estagio', 'Vendedor', 'Valor', 'Criado'];
        rows = db.queryAll('SELECT id, provedor_nome_lead, estagio, responsavel_vendedor, valor_estimado, criado_em FROM vendas_negocios ORDER BY criado_em DESC');
    } else if (tipo === 'treinamentos') {
        headers = ['ID', 'Provedor', 'Titulo', 'Data', 'Status'];
        rows = db.queryAll('SELECT t.id, p.nome as provedor, t.titulo, t.data_treinamento, t.status FROM treinamentos t JOIN provedores p ON t.provedor_id = p.id ORDER BY t.data_treinamento DESC');
    ...
    rows.forEach(r => {
        csvLines.push(Object.values(r).map(v => `"${(v || '').toString().replace(/"/g, '""')}"`).join(';'));
    });
...
app.put('/api/whatsapp-ia/config', requireAdmin, (req, res) => {
    const db = getDB();
    const { ativo, provedor_ia, modelo, api_key, prompt_sistema, max_tokens, temperatura, contexto_kb, auto_responder } = req.body;
    const existing = db.queryGet('SELECT id FROM whatsapp_ia_config ORDER BY id DESC LIMIT 1');
    if (existing) {
        let sql = 'UPDATE whatsapp_ia_config SET ativo = ?, provedor_ia = ?, modelo = ?, prompt_sistema = ?, max_tokens = ?, temperatura = ?, contexto_kb = ?, auto_responder = ?';
        const params = [ativo ? 1 : 0, provedor_ia || 'openai', modelo || 'gpt-3.5-turbo', prompt_sistema || '', Number(max_tokens) || 500, Number(temperatura) || 0.7, contexto_kb ? 1 : 0, auto_responder ? 1 : 0];
        if (api_key && !api_key.includes('...')) { sql += ', api_key = ?'; params.push(api_key); }
        sql += ' WHERE id = ?';
...
function buscarDadosTitularLocal(db, { documento, nome }) {
    const resultado = { provedores: [], chamados: [], negocios: [], formularios: [], consentimentos: [], atividades: [] };
    ...
    if (nome) {
        const porNome = db.queryAll('SELECT * FROM provedores WHERE nome LIKE ? OR responsavel LIKE ?', [`%${nome}%`, `%${nome}%`]);
    ...
    for (const p of resultado.provedores) {
        const chamados = db.queryAll('SELECT id, titulo, descricao, categoria, status, prioridade, criado_em, data_resolucao FROM chamados WHERE provedor_id = ?', [p.id]);
    ...
    for (const p of resultado.provedores) {
        const neg = db.queryAll('SELECT id, provedor_nome_lead, contato_lead, estagio, plano_interesse, valor_estimado, criado_em FROM vendas_negocios WHERE provedor_id = ?', [p.id]);
        for (const n of neg) {
    ...
    for (const p of resultado.provedores) {
        const cons = db.queryAll("SELECT * FROM lgpd_consentimentos WHERE entidade_tipo = 'provedor' AND entidade_id = ?", [p.id]);
            ...
            if (encontrados.length) {
                ...
                resultados.push({
                    ...
                    clientes: encontrados.map(c => {
                        const contratosCliente = contratos.filter(ct => String(ct.cliente_id_externo) === String(c.id_externo));
                        return {
                            nome: c.nome,
                            ...
                            endereco: c.endereco,
                            status: c.status,
                            contratos: contratosCliente.map(ct => ({ plano: ct.plano, status: ct.status, valor: ct.valor }))
                        };
                    })
                });
            }
...
app.get('/api/lgpd/exportar-dados', requireAdmin, async (req, res) => {
    ...
    for (const config of erpsAtivos) {
        ...
        try {
            ...
            if (encontrados.length) {
                dadosERP.push({ erp: config.tipo, label: adapter.label, clientes: encontrados.map(c => ({ nome: c.nome, documento: c.documento, email: c.email, telefone: c.telefone, endereco: c.endereco, status: c.status })) });
            }
        } catch {}
    }
    ...
    const exportacao = {
        ...
        dados_locais: {
            provedores: dadosLocais.provedores.map(p => ({
                nome: p.nome, cnpj: p.cnpj, email: p.email, telefone: p.telefone,
                endereco: p.endereco, responsavel: p.responsavel, plano: p.plano, criado_em: p.criado_em
            })),
            chamados: dadosLocais.chamados,
            negocios: dadosLocais.negocios,
            formularios: dadosLocais.formularios.map(f => {
                try { return { ...f, dados: JSON.parse(f.dados || '{}') }; } catch { return f; }
            }),
...
app.post('/api/lgpd/anonimizar', requireAdmin, (req, res) => {
    ...
    if (!escopo || escopo === 'completo') {
        // Anonimizar chamados
        const chamadosCount = db.queryGet('SELECT COUNT(*) as total FROM chamados WHERE provedor_id = ?', [provedor_id]);
        ...
        // Anonimizar negocios
        db.queryRun("UPDATE vendas_negocios SET contato_lead = '[REMOVIDO]', observacoes = '[REMOVIDO]' WHERE provedor_id = ?", [provedor_id]);
    ...
    registrarAtividade(req, 'anonimizacao', 'lgpd', provedor_id, `Dados anonimizados: ${nomeOriginal} (escopo: ${escopo || 'completo'})`);
    res.json({ sucesso: true, registros_afetados: registrosAfetados, mensagem: `Dados de "${nomeOriginal}" foram anonimizados` });
});
...
app.post('/api/lgpd/consentimento', requireAdmin, (req, res) => {
    ...
    registrarAtividade(req, 'consentimento', 'lgpd', result.lastInsertRowid, `${tipo_consentimento}: ${consentido ? 'concedido' : 'revogado'}`);
...
app.post('/api/lgpd/retencao', requireAdmin, (req, res) => {
    ...
    const result = db.queryRun('INSERT INTO lgpd_retencao (tabela, campo, tempo_retencao_dias, acao) VALUES (?, ?, ?, ?)',
...
app.get('/api/lgpd/relatorio', requireAdmin, (req, res) => {
    ...
    const relatorio = tabelas.map(t => {
        ...
        } catch { return { tabela: t, registros: 0, retencao: null }; }
    });
    ...
    res.json({ tabelas: relatorio, total_consentimentos: consentimentos?.total || 0, total_anonimizados: anonimizados?.total || 0 });
});
...
app.post('/api/integracoes-externas', requireAdmin, (req, res) => {
    ...
    const result = db.queryRun('INSERT INTO integracoes_externas (tipo, nome, config, ativo) VALUES (?, ?, ?, ?)',
        [tipo, nome || tipo, typeof config === 'object' ? JSON.stringify(config) : (config || '{}'), ativo !== undefined ? Number(ativo) : 1]);
...
app.put('/api/integracoes-externas/:id', requireAdmin, (req, res) => {
    ...
    db.queryRun('UPDATE integracoes_externas SET tipo = ?, nome = ?, config = ?, ativo = ? WHERE id = ?',
        [tipo, nome, typeof config === 'object' ? JSON.stringify(config) : (config || '{}'), ativo !== undefined ? Number(ativo) : 1, Number(req.params.id)]);
...
app.put('/api/sla/config/:id', requireAdmin, (req, res) => {
    ...
    db.queryRun(
        ...
        [categoria, prioridade || 'normal', Number(tempo_resposta_horas), Number(tempo_resolucao_horas), ativo !== undefined ? Number(ativo) : 1, Number(req.params.id)]
    );
...
app.get('/api/sla/dashboard', requireAuth, (req, res) => {
    ...
    const estourados = db.queryGet("SELECT COUNT(*) as total FROM chamados WHERE sla_estourado = 1 AND status NOT IN ('resolvido','fechado')") || { total: 0 };
    const dentro = db.queryGet(`SELECT COUNT(*) as total FROM chamados WHERE sla_resolucao_limite IS NOT NULL AND sla_resolucao_limite > ? AND sla_estourado = 0 AND status NOT IN ('resolvido','fechado')`, [agora]) || { total: 0 };
    const criticos = db.queryGet(`SELECT COUNT(*) as total FROM chamados WHERE sla_resolucao_limite IS NOT NULL AND sla_resolucao_limite <= datetime(?, '+2 hours') AND sla_estourado = 0 AND status NOT IN ('resolvido','fechado')`, [agora]) || { total: 0 };
...
function calcularSLA(categoria, prioridade) {
    const db = getDB();
    const regra = db.queryGet(
        'SELECT * FROM sla_config WHERE categoria = ? AND prioridade = ? AND ativo = 1',
        [categoria, prioridade || 'normal']
    );
    if (!regra) {
        // Fallback: buscar regra 'normal' da categoria
        const fallback = db.queryGet(
            'SELECT * FROM sla_config WHERE categoria = ? AND prioridade = ? AND ativo = 1',
            [categoria, 'normal']
        );
        return fallback;
...
function processarSLAEstourados() {
    try {
        ...
        for (const ch of estourados) {
            ...
            criarNotificacaoParaPerfil('admin', 'sla', 'SLA Estourado',
                `Chamado #${ch.id} "${ch.titulo}" (${ch.provedor_nome}) estourou o SLA`, '/chamados');
        }
    } catch (e) {
...
function processarFollowUps() {
    try {
        ...
        const cfg = db.queryGet("SELECT * FROM vendas_followup_config WHERE tipo = 'proposta_sem_resposta' AND ativo = 1");
        if (cfg) {
            const dias = parseInt(cfg.dias_apos, 10) || 3;
            const propostas = db.queryAll(`SELECT * FROM vendas_propostas WHERE status = 'enviada' AND atualizado_em <= datetime('now','localtime','-' || ? || ' days')`, [dias]);
            for (const p of propostas) {
                const existe = db.queryGet("SELECT id FROM vendas_tarefas WHERE titulo LIKE ? AND status = 'pendente'", [`%Follow-up proposta #${p.id}%`]);
                if (!existe) {
                    db.queryRun("INSERT INTO vendas_tarefas (titulo, descricao, tipo, data_hora, responsavel) VALUES (?, ?, 'follow_up', datetime('now','localtime'), ?)",
                        [`Follow-up proposta #${p.id} - ${p.provedor_nome}`, (cfg.mensagem || '').replace('{dias}', dias).replace('{provedor}', p.provedor_nome), p.criado_por || 'Sistema']);
                }
            }
        }
        ...
        if (cfgParado) {
            ...
            for (const n of negocios) {
                ...
                const existe = db.queryGet("SELECT id FROM vendas_tarefas WHERE titulo LIKE ? AND status = 'pendente'", [`%Follow-up negocio #${n.id}%`]);
                if (!existe) {
                    const msg = (cfgParado.mensagem || '').replace('{negocio}', nomeNegocio).replace('{dias}', dias);
                    db.queryRun("INSERT INTO vendas_tarefas (titulo, descricao, negocio_id, tipo, data_hora, responsavel) VALUES (?, ?, ?, 'follow_up', datetime('now','localtime'), ?)",
                    ...
                    if (user) {
                        db.queryRun("INSERT INTO notificacoes (usuario_id, tipo, titulo, mensagem, link) VALUES (?, 'follow_up', ?, ?, '/vendas')",
        ...
        if (cfgSemAtiv) {
            ...
            for (const n of negocios) {
                ...
                const nomeNegocio = n.provedor_nome_lead || n.provedor_nome || `Lead #${n.id}`;
                const existe = db.queryGet("SELECT id FROM vendas_tarefas WHERE titulo LIKE ? AND status = 'pendente'", [`%Sem interacao #${n.id}%`]);
                if (!existe) {
                    const msg = (cfgSemAtiv.mensagem || '').replace('{negocio}', nomeNegocio).replace('{dias}', diasSem);
                    db.queryRun("INSERT INTO vendas_tarefas (titulo, descricao, negocio_id, tipo, data_hora, responsavel) VALUES (?, ?, ?, 'follow_up', datetime('now','localtime'), ?)",
        ...
        } catch (e) { /* NPS auto-create silently */ }
    } catch (e) {
